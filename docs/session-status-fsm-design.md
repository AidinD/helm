...
- Increment 3 (done): ALL remaining needs-you surfaces migrated - the fleet-card badge (`wrapped` -> "done") + accent, and the OS attention toast (+ its `previouslyWaiting` transition set). `classifierSaysSessionDone` is now unused and was deleted. Every needs-you surface reads the one `lifecycleState` field. Behaviour-preserving EXCEPT one intentional change surfaced by an independent review: keying `previouslyWaiting` on lifecycleState (not status) means the OS toast now fires on a `wrapped -> waiting` transition (the async classifier flipping "done" back to "waiting_for_input" while status stays "waiting"). The old status-keyed set swallowed that as a false negative; firing it aligns with prefer-false-positives. Kept intentionally; documented at the call site.
