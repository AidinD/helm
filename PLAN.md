...
- **The fleet spend ceiling is now an explicit, off-by-default Settings toggle** ("Fleet guardrail") instead of a capability that only ever existed as an unreachable `setCeiling(null)` call - matches Aidin's subscription use (nothing to cap by default).
