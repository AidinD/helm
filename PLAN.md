...
What landed later the same day, 2026-08-12 (see DECISIONS.md for the reasoning behind each):
- **Two second-mate ownership bugs fixed:** archiving a session now frees any second-mate binding pointing at it (a fresh second mate could otherwise resurrect an archived one), and jumping into a second mate no longer adopts a session another node (Captain, or a different first mate's second mate) already owns.
- **The review page's remaining gaps closed:** a record-less card (most commonly an autopilot run) now gets a real body - autopilot writes a `judgment` record from its own result instead of leaving the card empty, and "see diff"/"independent reviewer" no longer require a record to exist at all; the "commits without a task" band's floor became a set of acknowledged commits instead of a single watermark, so two commits on divergent branches clear independently instead of toggling forever.
- **A stashed WIP from the 2026-08-11 session was recovered, not re-derived:** the review page's "Bound to commits" filter and the quota widget's Resume/Stop button, both already complete, verified against the fast suite, and committed.
- **The fleet spend ceiling is now an explicit, off-by-default Settings toggle** ("Fleet guardrail") instead of a capability that only ever existed as an unreachable `setCeiling(null)` call - matches the captain's subscription use (nothing to cap by default).
- **The dispatch queue's acks/reports get a pruner:** 44/24 files had accumulated since 2026-07-16 with nothing ever removing one, the same unbounded-growth shape as the `.tmp`-file leak fixed earlier the same week. Ages off past 14 days on a daily sweep; requests are deliberately excluded (a lingering one signals a stuck claim, not routine turnover).
- **Handled a live two-writer collision on the same working tree** (a concurrent session editing `renderer.js`/`preload.cjs`/`main.js` while this session was committing its own changes) by staging only the owned hunks via `git apply --cached` rather than a bulk `git checkout` - both sessions' work landed intact.

**Performance: the app-wide lag, measured and fixed in four phases (2026-08-12).**
The captain: "jag tycker helm är lite långsamt ibland, att byta mellan vyer och sessioner och ibland laggar input till".
Everything below is measured on his own machine and board, before and after; see DECISIONS.md for what was rejected and why.
- **The periodic freezes are gone.** The review queue started one git process per review row, and a git process costs ~70ms of startup on Windows before doing any work: 21 rows = 2193ms of blocked main process, and the review badge paid it EVERY MINUTE because its cache allowed 20s while the badge ticked at 60s. Batched into one `git log` (114ms, identical results verified sha-for-sha), and the cache is now keyed on its real inputs (0.4ms per tick). The stale-build pill no longer starts git every 45 seconds (70ms -> 1.3ms, and zero processes in a packaged build).
- **The heavy work left the main process.** The review build and the session read run in an Electron `utilityProcess`. A trivial IPC issued DURING a full review build now answers in 2ms; the same measurement read 421ms when the build ran on the main thread. Every call keeps an in-process fallback, so a worker that cannot spawn makes Helm slower, never broken - and `heavyWorker:status` exposes which path is live, because a silent fallback is a speed-up you cannot tell has stopped.
- **Sessions are no longer re-read when nothing changed.** 93ms -> 56ms, with an identical status mix. An archived session's transcript is not read at all (its status could never depend on it, and 80 of 90 sessions are archived).
- **Typing and view switching.** `autoSizeComposer` measured the whole pane on every keystroke, forcing a layout of the entire transcript; a `ResizeObserver` tracks it instead (30 keystrokes: 60 pane measurements before, 0 after). The Review page painted nothing for 2770ms while it waited on a build - it now paints from the last known queue immediately, says it is checking, and repaints when the fresh build lands. Every other view was already under 25ms after the phases above.
- **A standing guard, so this cannot regress invisibly a third time:** every `ipcMain` handler is timed and any that blocks the main process past 25ms names itself in the log.
- **Still open:** the review build itself is ~1.4s warm (and multi-second on a first run, when every project's commit baseline is established) - it no longer blocks anything, but it is the next thing to make cheaper if the Review page's fresh repaint ever feels slow. The transcript index's 5-second TTL costs a 119ms rebuild and was deliberately left alone: it now runs in the worker, and raising it would trade a real correctness guard (a transcript that moved between project folders) for churn nobody experiences.
...
- **MCP:** first mate stays lean by design (the captain confirmed); second mates now pre-approve the user's own MCP servers so they're actually usable in headless -p (parity fix); regular sessions were already fine.
...
Remaining is still DE-RISKING, not building: prove it in real daily use (the captain is now running the installed app + feeding back p0s, which this session worked through). Parked: Google Calendar, interactive Plan prototypes, first-mate voice, collapsible settings, auto-update token.
...
- Authoritative daily loop settled with the captain: start in first mate -> it CREATES a real second-mate session per topic -> dual mode (orchestrate via first mate = more tokens, OR jump into a second mate directly = cheaper) -> ask first mate for a summary -> retire.
...
- Phase-2 build plan written (create real second-mate sessions, second-mate-dispatches-crew, report-up, first-mate-driven mode, model-per-tier) with caps + token budget + kill switch first. NOT built yet - awaiting the captain's go on the plan.
...
files). The captain confirmed it gave him a much clearer picture and wants
...
- **Verification gate before "done" (Simon Willison + the captain's own rule):** a
...
- **`kickoff` may need to evolve, not stay separate.** It currently hands off
  ...
  rooting mechanism existed. Once Helm is the captain's primary way of starting
...
reinvent it? This is a decision for the captain, not something to presume.
...
- **Firstmate only runs on macOS/Linux** (hard `tmux` + POSIX `stat`/`ps`
  dependency throughout, no Windows path) — a hard blocker on the captain's actual
...
**DECIDED (2026-07-03, the captain confirmed):** firstmate → pattern only (option
...
**Worktree note (resolves a question the captain raised: does Helm's own
...
**Self-hosting hazard (the captain raised this, worth designing around before
...
- **gnhf** ("good night, have fun") — long-running goal orchestrator:
  ...
  **A minimal FIRST-PASS UI now exists too** (`goalOrchestrator.js` is no
  ...
  UI is explicitly a DRAFT for the captain to react to, verified by wiring
  ...
  about WHEN to escalate to the captain, only fixed stop conditions), single
