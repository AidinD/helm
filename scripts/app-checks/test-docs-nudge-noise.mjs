// The four fixes from the captain's review of the docs-drift nudge (task 0831417b,
// 2026-07-28). His verdict: "jag vet inte hur jag ska använda den" - of four rows
// on his board exactly ONE was actionable.
//
//   tidepool 252 behind   -> work repo, on leave, can't act
//   loom 15 behind             -> actionable, his own
//   Documents/Claude       -> "unknown" - but it has NO version control at all
//   nw-snackables        -> "unknown" - work repo
//
...
// Docs, but no version control at all - the captain's notes folder.
...
process.env.HELM_E2E_PORT = process.env.HELM_E2E_PORT || "9380";
...
try {
  ...
  const entries = [
    ...
    { key: "d:\\repo\\tidepool", cwd: "D:\\Repo\\tidepool", touchedAt: now - 5 * day },
  ...
  const withPark = docsNudgeCandidates(entries, { now, parked: ["D:\\Repo\\Tidepool"] });
  ok(withPark.parked === 1, `a parked project is excluded (${withPark.parked})`);
  ok(
    !withPark.candidates.some((c) => /tidepool/i.test(c)),
  ...
  // the captain, twice: "jag vet inte hur jag ska använda den", then "jag vet
