...
//    error, no hint. Aidin: "Trycker på send back, inget händer." Silence is the
...
//    the one band that fills up was the only one with no way to clear it. Aidin:
...
try {
  ...
  ok(/\[Aidin \$\{new Date\(\)\.toISOString\(\)\.slice\(0, 10\)\}\] \$\{note\}/.test(handler), "source: with a dated, attributed note for the Jot card");
