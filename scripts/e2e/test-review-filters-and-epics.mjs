...
//
// Aidin, 2026-08-04, two problems and three asks:
...
const ROWS = `[
  ...
  { taskId: "bbbbbbbb-0000-4000-8000-000000000001", title: "Lone crewline task", category: "Crewline", repoPath: "D:\\\\Repo\\\\tgs-crewline", parentId: null, verdict: "unrecorded", band: "unrecorded", problems: ["nothing"], caveats: [] },
...
try {
  ...
  ok(/Lone crewline task/.test(codeOnly.text), "a board whose name only PARTLY matches its repo folder still counts (Crewline -> tgs-crewline)");
  ok(
  ...
  const filtered = await app.eval(`window.__revProbe(window.__rows, true, "Crewline")`);
  ok(/Lone crewline task/.test(filtered.text) && !/The epic/.test(filtered.text), "picking a project shows only that project");
  ok(filtered.rowsTotal === 1, `one row for Crewline (${filtered.rowsTotal})`);
  ok(
    filtered.chips.some((c) => /^helm/.test(c)),
    `the OTHER projects' chips are still there, so there is a way back (${filtered.chips.join(" | ")})`
  );
  ok(filtered.activeChips.some((c) => /Crewline/.test(c)), `the active chip is marked (${filtered.activeChips.join(" | ")})`);
  ok(
