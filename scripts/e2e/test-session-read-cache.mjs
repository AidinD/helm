...
// session's transcript to work out whose turn it was - 2.4MB per tick on Aidin's machine,
...
try {
  ...
  ok(
    ...
    `appending an assistant turn CHANGES the answer (${after.lastRole}) - if this ever reads "user", a session waiting for Aidin has gone invisible in the needs-you queue`
  );
