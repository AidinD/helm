...
const history = [
  { dispatchedBy: "mate_A", projectPath: "D:/Repo/tgs-crewline", status: "running", updatedAt: 300 },
  { dispatchedBy: "mate_A", projectPath: "D:/Repo/tgs-reinmaker", status: "done", commitCount: 3, branchName: "b1", updatedAt: 200 },
...
assert(fs.dispatched[0].project === "tgs-crewline" && fs.dispatched[0].updatedAt === 300, "dispatched runs are newest-first");
assert(fs.dispatched.every((d) => typeof d.project === "string" && !d.project.includes("/")), "project is the basename, not a full path");
const rein = fs.dispatched.find((d) => d.project === "tgs-reinmaker");
...
assert(fs.liveByProject["tgs-crewline"] === 1 && !fs.liveByProject["tgs-reinmaker"], "liveByProject rolls up only running runs");
assert(fs.needsCaptainByProject["jot"] === 1 && fs.needsCaptainByProject["tgs-reinmaker"] === 1, "needsCaptainByProject rolls up runs awaiting the captain");
