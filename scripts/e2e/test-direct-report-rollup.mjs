...
try {
  ...
  // review, plus a Direct session node, and render the fleet.
  await app.eval(`(() => {
    goalRuns.clear();
  ...
  const direct = await app.eval(`!!document.querySelector("#dashFleetSlot .fleet-mate-card.direct")`);
  assert(direct, "the Direct card renders");

  const rollups = await count("#dashFleetSlot .fleet-mate-card.direct .fleet-report-rollup");
  assert(rollups === 1, `the Direct card shows a report roll-up for the captain's own runs (got ${rollups})`);

  const rollupText = await app.eval(`(() => { const el = document.querySelector("#dashFleetSlot .fleet-mate-card.direct .fleet-report-rollup"); return el ? el.innerText : ""; })()`);
  ...
  await app.eval(`(() => { document.querySelector("#dashFleetSlot .fleet-mate-card.direct .fleet-report-rollup-head").click(); return true; })()`);
  await app.eval("new Promise(r => setTimeout(r, 200))");
  const rows = await count("#dashFleetSlot .fleet-mate-card.direct .fleet-report-rollup .dash-queue-row");
