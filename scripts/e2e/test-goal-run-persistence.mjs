...
// (one "done", one stale "running" whose process is gone), launches Maestro,
...
const fixture = [
  { goalRunId: "persist-done-1", goal: "A finished run", projectPath: "X", status: "done", worktreePath: "wt/a", branchName: "maestro/goal-a", commitCount: 2, stoppedReason: "completed", escalation: null, error: null, startedAt: now, updatedAt: now },
...
try {
  ...
  const hist = await app.eval(`window.maestro.getGoalRunHistory()`);
