...
// A real folder the NAME GUESS would resolve "Skiff" to, so the precedence check below
...
const GUESSABLE = path.join(tmp, "nw-skiff-guess");
...
try {
  ...
  const roots = repoRootedCategories(
    [], // no records
    [{ cwd: GUESSABLE }], // a session cwd whose basename matches the "Skiff" board by name
    [] // no goal runs
  );
  ok(roots.repoFor("Skiff") === GUESSABLE, "the name guess still resolves a board to a repo folder whose basename contains it");
