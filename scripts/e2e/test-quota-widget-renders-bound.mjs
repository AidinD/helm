// What the Quota widget ACTUALLY SHOWS, with Aidin's own data in it.
//
...
try {
  ...
  const res = await app.eval(`(() => {
    const HOUR = 3600000;
    const now = Date.now();
    // Aidin's real windows, as read from C:\\Users\\aidin\\.helm\\config.json:
...
console.log(
  exit === 0
    ? "VERIFY OK: with Aidin's real data the widget says '≥36%' and 'as of 39h ago' on the surface he looks at, and never a bare 36%."
    : "VERIFY FAILED."
);
process.exit(exit);
