...
// scripts/e2e launch the real Electron app with a window; a build machine has no
...
// Run:  node scripts/e2e/test-ci-evidence-honest.mjs
...
const e2eDir = path.join(repo, "scripts", "e2e");
...
const split = classifySuite(e2eDir);
...
// An independent second implementation of the rule, on purpose. The point of the
// coverage statement is that its numbers are true; a single implementation of the
// classifier could be wrong in the same way twice and nothing would notice.
{
  const files = fs.readdirSync(e2eDir).filter((f) => f.startsWith("test-") && f.endsWith(".mjs"));
  let mine = 0;
  for (const f of files) {
    const src = fs.readFileSync(path.join(e2eDir, f), "utf8");
    // "imports the CDP harness" is what makes a check launch the app - the same rule
    // scripts/run-tests.mjs uses to decide which lane a file belongs to.
    if (/^\s*(?:import\s|const\s*\{[^}]*\}\s*=\s*await\s+import\()[^\n]*harness\.mjs/m.test(src)) {
      mine += 1;
    }
  }
  ok(mine === split.app, `a second, independent count of the app lane agrees (${mine} vs ${split.app})`);
  ok(files.length === split.total, `and of the suite size (${files.length} vs ${split.total})`);
}
  ...
  for (const [file, { pkg, why }] of Object.entries(EXCLUDED)) {
    ...
    if (!reaches) {
      for (const m of src.matchAll(/from\s+"(\.\.\/\.\.\/src\/[^"]+)"/g)) {
        const dep = path.join(e2eDir, m[1]);
