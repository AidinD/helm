...
// opened fresh. The bug (found by following a live Reinmaker session): jumpIntoSecondMate set
...
try {
  ...
  const res = await app.eval(`(() => {
    ...
    // second mate - exactly the Reinmaker case). secondMateBySessionId is empty, so the OLD
    // resume path left secondMateId undefined here.
    state.sessions = [{ sessionId: "s-rein", cliSessionId: "c-rein", cwd: ${JSON.stringify(PROJECT)}, title: "a running reinmaker chat", status: "idle", isArchived: false, lastActivityAt: 2 }];
    secondMateBySessionId = new Map();
    mateBySessionId = new Map();
    jumpIntoSecondMate({ secondMateId: "sess_c-rein", projectPath: ${JSON.stringify(PROJECT)}, sessionId: "c-rein", name: "Reinmaker" });
    const p = panes[0] || {};
