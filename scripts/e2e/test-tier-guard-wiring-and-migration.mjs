...
seed({ "sess_S1": { sessionId: "S1", status: "created", name: "Crewline planning" } });
...
ok(after[REAL]?.sessionId === "S1" && after[REAL]?.name === "Crewline planning", `and carries its session AND the name the captain typed (${JSON.stringify(after[REAL])})`);
...
// matches dispatchedBy exactly, and eleven of the crewline second mate's reports silently
...
// NAME ("tgs-crewline"), not a path. A first draft preferred that field, and it hashed to a
...
const fromName = secondMateId(DIRECT_FIRST_MATE, "tgs-crewline");
