...
// rather than silent.
//
// Run:  node scripts/pure-checks/test-live-checks-declared.mjs
...
// BOTH lanes. This used to read its own directory, which was the whole suite until the
// 2026-09-03 split - after it, scanning one folder silently narrowed the scan to half the
// checks while still reporting a confident count. A gate that spends tokens is exactly the
// thing that must not be half-scanned, so the lanes are named rather than inferred from
// where this file happens to live.
const LANES = ["pure-checks", "app-checks"].map((name) => path.join(dir, "..", name));
const files = LANES.flatMap((lane) =>
  fs
    .readdirSync(lane)
...
// Reported by basename (two lanes, one readable list), read by full path.
const fullPath = new Map(files.map((f) => [path.basename(f), f]));
...
for (const file of files) {
  const verdict = classify(fs.readFileSync(file, "utf8"));
  if (verdict.state === "declared") {
    declared.push(path.basename(file));
  } else if (verdict.state === "exempt") {
    exempt.push({ file: path.basename(file), why: verdict.exemption });
  } else if (verdict.state === "undeclared") {
    undeclared.push({ file: path.basename(file), why: verdict.why, hasWeakExemption: verdict.hasWeakExemption });
  }
}
  ...
  console.log(
    `      ${u.file}: ${u.why}.` +
      (u.hasWeakExemption
        ...
        : ' Add `import { requireLive } from "../checks-lib/live-gate.mjs";` + requireLive("<what it does>"), or a `// LIVE-EXEMPT: <reason>` line if it only stubs the CLI.')
  );
}
...
for (const file of declared) {
  const lines = fs.readFileSync(fullPath.get(file), "utf8").split("\n");
...
const ungatedSample = 'import { launch } from "../checks-lib/harness.mjs";\nconst app = await launch();\n' + EVAL_CALL;
const gatedSample = 'import { requireLive } from "../checks-lib/live-gate.mjs";\nrequireLive("starts a real session");\n' + ungatedSample;
