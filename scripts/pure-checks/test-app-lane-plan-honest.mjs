...
const APP_DIR = path.join(repo, "scripts", "app-checks");
...
// The lane, counted independently of the plan. Since the split the lane IS the folder, so
// "independently" now means reading the directory rather than trusting the plan's own total -
// the plan could still drop a file from its list, and this is what would notice.
// That the folder matches BEHAVIOUR is a different question, proved by
// test-lane-folders-tell-the-truth.mjs rather than repeated here.
const lane = fs
  .readdirSync(APP_DIR)
