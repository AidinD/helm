...
requireLive("drives a real first-mate session on Sonnet through the fleet-state tool");
...
// work in "halyard". A correct survey should surface halyard as NOT mine.
writeFleetState(
  metaHome,
  assembleFleetState(
    [{ mateId: "mate_me", name: "Nemo", slot: 0 }, { mateId: "mate_other", name: "Barbossa", slot: 1 }],
    [
      { dispatchedBy: "mate_me", projectPath: "D:/Repo/skiff", status: "running", updatedAt: 3 },
      { dispatchedBy: "mate_other", projectPath: "D:/Repo/halyard", status: "running", updatedAt: 2 },
    ],
    Date.now()
  )
);
...
try {
  ...
  let reply = "";
  for (let attempt = 0; attempt < 4 && !/halyard/i.test(reply); attempt++) {
  ...
  assert(!/TOOL-BLOCKED/i.test(reply) && /halyard/i.test(reply), "the real first-mate session called helm_fleet_state through the permission gate and named the OTHER mate's in-flight project (halyard)");
