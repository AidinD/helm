 ...
 * Aidin's real board, measured 2026-08-12 - and it was being rebuilt once a minute by a
 ...
 * would be a gate nobody can clear: Aidin does not hand-write a sentence per task, and a
...
function criticalityProblems(rec) {
  ...
  // Aidin, 2026-08-20: "ändringar som påverkar eventuell säkerhet eller integritet ska
...
export function recordCaveats(rec) {
  ...
  if (!intent) {
    ...
    caveats.push(`What was asked is stated in my words, not Aidin's - ${intentSourceNote(intent.source).toLowerCase()}`);
  }
...
export function recordCheckRun(metaHome, taskId, run, { now = Date.now(), pinnedHead = null } = {}) {
  ...
  // pinnedHead lets a caller that ran the check in an ISOLATED worktree (Aidin,
...
export function buildReviewQueue(reviewTasks, records, metaHome = null) {
  ...
  const rows = (reviewTasks || []).map((t) => {
    ...
    return {
      ...
      // Did the ask itself move after the record snapshotted it? Usually this means Aidin
