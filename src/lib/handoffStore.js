 ...
 * categories that already exist. MATCH-FIRST by design (the captain: "se om vi kan
...
export function resolveHandoffCategory(proposed, existing = [], fallback = "general") {
  ...
  // was the one moment no matching ran at all (the captain, 2026-08-02).
  ...
  for (const candidate of known) {
    ...
    if (shorter.length > 0 && shorter.every((w) => longer.includes(w))) {
      ...
      // the captain's LEADERSHIP development into the topic holding his PHYSICAL
