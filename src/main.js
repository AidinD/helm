...
// minute, because the cache was keyed on the wrong thing. Both times the symptom Aidin
...
ipcMain.handle("context:saveHandoff", async (_event, { cwd, text, title, category } = {}) => {
  ...
  try {
    ...
    // Commit it immediately (Aidin, 2026-08-07: "ja, en handoff borde commitas
...
function applySessionArchiveInner(sessionId, archived) {
  ...
  // starting fresh (Aidin, 2026-08-12: "den 2nd mate som spann upp bindades till en
  ...
  if (shouldArchive) {
    try {
      ...
      for (const form of sessionIdForms(sessionId)) {
        ...
        // (Aidin, 2026-08-12). Found by review, 2026-08-16.
 ...
 *
 * Aidin hit exactly that. His "Träning och kost (Hevy)" node was parked before
...
ipcMain.handle("auth:login", async () => {
  ...
  return await new Promise((resolve) => {
    ...
    let child;
    try {
      // --claudeai: the subscription flow (matches how Aidin is signed in -
...
function holdBack(jotConfig, todo, reason) {
  ...
  if (!res.ok) {
    ...
    // in @jot/core the same day. Aidin hit exactly this (2026-08-03): "jag körde run
...
async function autoCaptainTick({ force = false } = {}) {
  ...
  try {
    ...
    for (const todo of act) {
      ...
      // GO.
      //
      // The shape here is the fleet's own hierarchy, and it is the shape Aidin
...
// Aidin found three by hand on 2026-08-03, one pointing at work merged in July.
//
...
function buildDispatchReport({ dispatchId, mateId, request, result, meta }) {
  ...
  return {
    ...
    // else surfaces unreviewed commits, and 117 of them reached crewline's master
    // unread.
...
// a board Aidin was not even looking at. An age gate answers "how old is this?" when the
...
// written onto the task, because it is a fact about what Aidin has read, not about the
// work.
...
ipcMain.handle("reviews:acknowledgeCommit", (_event, { projectPath, sha, shas } = {}) => {
  ...
  try {
    ...
    // acking one used to re-surface the other forever (Aidin, 2026-08-12). listUnboundCommits
...
// session shows up without an app restart (Aidin already restarts Helm after updates - the
...
ipcMain.handle("reviews:runChecks", async (_event, { taskId } = {}) => {
  ...
  // Bind the whole run to ONE commit (Aidin, task 76790f23: "Bind varje review
...
function repairDisplayKeyBindings() {
  try {
    ...
    // the crewline second mate silently stopped being collectable the moment the binding
...
function repairDisplayKeyReports(projectPathForSession) {
  ...
  for (const name of names) {
    ...
    // NAME ("tgs-crewline"), not a path, so hashing it produced a valid-looking id for a
