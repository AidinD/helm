...
## 2026-09-08 - A guard is attached when it starts, not when its filename is on disk

`main.js` decided the tier guard was attached by asking whether the hook script existed. It did. Behind it, `tierGuard.js` imported a `personas.js` that `build.extraResources` never shipped, so the CLI spawned the hook, plain node died with `ERR_MODULE_NOT_FOUND`, and the harness got exit 1 with an empty stdout - which is a hook that crashed, and a crashed `PreToolUse` hook denies nothing. Six days of installed builds from v0.2.268 had first mates that every layer of the app described as supervised and nothing was supervising. The `existsSync` check was green the whole time, because the file it named was there.

**The lesson is not "check more files."** The packaging bug now has a build-time check that resolves the hook's import graph, which is right and is not enough: any list of files can be short by one, and being short by one is the entire bug. So the app asks the only question whose answer cannot be wrong by omission - it runs the guard.

`lib/tierGuardProbe.js` spawns the hook and hands it a call the policy **must** refuse, then requires the refusal back. That is deliberately stronger than exit 0: a hook that loads and answers nothing also exits 0, and in this protocol silence means allow, so "exit 0" would be passed by a guard that is running and inert. Only a hook that started, read its payload, reached the policy and came back with a real denial counts as attached. A **success** is memoised for the app run, the way the runner already is - the import graph cannot change under a running process. A **failure is not the same kind of fact**: "cannot load its imports" never heals, "the spawn returned EPERM" does, and remembering the second one for the life of the process would leave a ban tier dead until Helm restarts on the strength of one bad tick. So a failure expires after a minute, with no attempt to sort transient causes from permanent ones - a classifier would be a guess and the expiry gets both right.

**The probe execs the hook; the CLI shells it.** That divergence is the one thing worth spelling out, because both directions of it are real and neither is obvious. A `type: "command"` hook is a STRING the CLI hands to a shell, so the faithful thing would be `shell: true` - and a synchronous spawn through a shell **cannot be bounded**: the timeout kills the shell, the grandchild keeps the pipes, and `spawnSync` waits forever. Measured, with and without redirecting the child's output to files. A probe that can freeze Helm's main process indefinitely is a worse bug than the one it was added to catch, so it execs, and the two gaps that opens are closed away from the probe:

- **A shell can run things a direct exec cannot** - a `.cmd`/`.bat` node shim (nvm, volta, an npm-global wrapper) fails with EINVAL. That is not a broken guard, it is a runtime the probe cannot ask about, and with a ban tier now refusing to launch it would have read as "dead" and taken every first mate on such a machine with it. So `tierGuardRunner` only ever returns a directly executable runtime, falling back to the app's own binary.
- **A shell can reparse what a direct exec would not.** Inside double quotes cmd leaves `&`, `^` and `|` alone; a quote and a percent sign are the two characters that reshape the line, and both halves of the command can come from an environment variable. `hookCommand` - the single place the string is built, used by the probe and by the launch config - refuses those rather than certifying a string it never shells.

**What happens when it does not start, and why that is not a coin-flip.** The hook already answers a version of this one layer down: when its classifier throws it **denies** for the tiers with no write budget and **allows** for the ones that have one. For a first mate or the assistant seat the guard is the only thing between the seat and the file system - `--disallowedTools` still refuses `Edit`/`Write`, but `cat > file` is not a tool name, and closing that hole is why the guard exists. For a second mate or crew it is a limiter over a session that may write anyway, so its absence costs an uncounted edit. Attach time takes the same split, or Helm has two rules for one question:

- **Ban tiers fail CLOSED.** The launch is refused, with the reason, in the pane, at the moment the captain tries. There is no supervised way to run those tiers without a guard, so pretending otherwise is the state this whole exercise exists to make impossible.
- **Budget tiers fail OPEN, loudly.** Refusing them would cost a day's work to enforce a limit whose absence costs an edit nobody counted.

**Which tiers those are is asked of the policy, not listed in `main.js`** - and asked as a **positive**, which is the whole of it. The first version asked "would this tier be refused a write", and that was the same bug wearing a new costume: `decideToolCall`'s own fall-through **allows** a tier it does not recognise, so a tier constant wired into a launch before its policy branch was written would have gone straight through unguarded - precisely the omission the paragraph was claiming to have closed. Caught by the independent pass, not by the author. It now asks the policy to say **yes**: launch without a guard only when the policy positively reports that this tier writes, which only a tier it actually recognises does. Naming what may proceed puts everything present and future on the refused side without anyone having to notice it appeared - the same inversion the assistant's advisory-seat allow list is built on. A hand-kept list of strict tiers would have been the same failure mode as a hand-kept list of shipped files, and the assistant seat has already been on the open side once by exactly that omission.

**The refusal travels as a thrown, tagged error rather than a flag on the returned config.** A call site that forgets to read a flag launches unguarded; a call site that forgets to catch does not launch at all. The safe direction should be the one you get by forgetting.

**And it reaches a human.** The only alarm this ever had was `console.error`, which in a packaged Electron app nobody reads - that is how six days went by. It now also comes back through `session:start`'s own error return and as a sticky notice on a `tierGuard:problem` channel, carrying the one line that names the problem (the full stack stays in the log; twenty lines of V8 frames in the corner of the UI is its own way of not being read).

**The suite that guards all of this was the weakest part of it, and that is worth recording as its own lesson.** An independent pass ran five mutations against the first version: four left both new checks green, including two that directly disarmed the probe. The checks looked thorough and were not, in the four ways this repository keeps rediscovering - a source scan that matched the guard **commented out**, an assertion about a turn lock the test never caused to be taken, an error branch nothing reached, and a whole branch (fail-open) with no coverage at all, which meant a probe wedged at `false` would have refused every first mate and disarmed every second mate with every check still green. Each is now covered, verified by re-running the same mutations and watching them go red. **Write the mutations before believing the suite** - a check that has never been made to fail is a check nobody has read.
...
**The mechanism is a tag, and not the persona field - which is the same question that produced the two-axes entry, asked a second time and answered the same way.** Putting the identity into the persona list makes "assistant" mutually exclusive with "red team", because a list where you pick one cannot hold two axes. That is not a hypothetical: it is the exact question that exposed the problem the first time, and reaching for the persona field now would re-create it a week after it was settled. So identity is a tag on the seat, temperament stays in `persona`, and the two are chosen separately.

**What the tag buys, beyond being correct about the axes.** Every one of the four kind-predicates becomes the same uniform lookup - the seat tagged X - rather than four places that each know a taxonomy. The second standing seat is a second tag rather than a third kind, so the named-seats decision needs no new machinery. And it repairs the addressing problem in the paragraph above instead of merely surviving it: a caller asks for the seat tagged assistant, so the seat can carry an ordinary name from the pool and stay reachable by what it *is*, which is what the caller meant in the first place.

**The one thing a tag does not give away free, and it has to be built rather than assumed.** A kind allowed exactly one standing seat by construction. A tag allows any number, so the lookup needs an explicit uniqueness rule wherever exactly one is required - and without it this is the `find()` hazard for the third time in a week: a predicate that stops having one answer keeps returning the first element and reads as working. Decide per tag whether it is singular, and make the violation loud rather than first-wins.
...
**What made the singular form correct, and what ended it.** The assistant seat was added as the one standing seat above every project, and singularity was not an arbitrary simplification: it was what the seat *was*. `isProjectPick` returns false when the picked folder is the meta-home, so a project seat can never be created there, and the creator takes a root it resolves itself rather than one a caller chooses. Both were right while the assistant was the only thing that belonged in that folder. A second standing role with its own manual - one that watches clocks and reports rather than holding conversations - has no way to exist under that shape. It is not a project, so the project path refuses it; it is not the assistant, so the singular path is already taken.

**The alternative that looks obvious is closed by an earlier decision.** Making the second role a persona on the existing seat is the cheap answer, and it contradicts the line that separates them. That line is *attended versus unattended*, and a seat cannot be both at once. This model already says a persona decides what a seat **is** rather than decorating it - the retire path carries that reasoning explicitly, because dropping a persona on refresh returned a standing assistant as a plain coordinator with the wrong manual and the wrong tools. So two roles that differ in whether a person is watching are two seats, not one seat with a switch.

**The migration is not "allow a second seat". It is retiring a question that no longer has one answer.** The accessor for the standing seat is a `find()` over "active, and of this kind". Under the old invariant that predicate had exactly one answer; under the new one it has several, and `find()` does not fail when its question becomes ambiguous - it returns whichever element is first and reads as working. Every caller then silently means "whichever meta-home seat happens to be earliest in the array". This is the third instance of one failure family in this codebase within a week: a persona default that returned a coordinator wearing the retired seat's root, a missing kind field that turned a refreshed project seat into a coordinator, and now an accessor whose uniqueness assumption is being removed out from under it. In each case nothing threw and nothing went red. **So the order of work is: make every caller say which seat it means, and only then permit a second one to exist.** Counting those callers is what decides whether this is a small change; permitting the second seat first would make the counting impossible, because the wrong answers would already be running.

**A binding that has to be re-read as a consequence.** The people store is attached by name to the standing seat and to no other tier - a session rooted in a project has no people store at all, deliberately, so that a seat cannot grow a second worse copy of a store that already exists. That binding names a *kind* that is about to have two members, and the second one is not obviously entitled to the same stores: a role that watches clocks and reports on progress has no conversation about people to hold. Shared root does not imply shared toolset, and the allowlist has to be able to express that even while only one member exists. This is cheap to allow for now and expensive to retrofit, which is the same reason the tier decision below was recorded before its migration rather than after.

**What this does not change.** Starting an ordinary session rooted in the meta-home was always possible and is unaffected; the refusal was only ever about creating a *seat* there. And the reason the meta-home is not a project pick stands - it is not a repository, it has no checkout, and one-seat-per-checkout is meaningless for it. Named meta-home seats are a third way of creating a seat, alongside picking a project and the pool, rather than a loosening of either.


## 2026-09-04 - The assistant seat writes documentation but never code, which rules out a pure MCP guard

The 2026-09-02 entry left two things open: which of the two write-guard resolutions to take, and four blurry cases. One of the blurry cases is now settled, and settling it constrains the resolution.

**The policy, stated as a line rather than a tool list.** The seat may read anywhere; it may write the personal stores; it may write documentation into another repository - a decision record, a plan correction, a README; it may never write code, or anything else in another repo's working tree. The reasoning is not effort or size. A wrong sentence in a decision record is visible to the next reader and costs a correction. A wrong line of code is invisible until it runs.

**Why this rules out the tidier of the two options.** The MCP-surface resolution was the better-argued one, and the argument still holds where it applies: a store can refuse a write that is in the right place and still wrong, which a path-aware guard cannot - a note carrying the wrong tag silently resets a cadence and turns an overdue duty green, and a guard that only asks "is this file under an allowed root" waves it through. But an MCP surface per store implies the seat has no file write at all, and writing documentation into a repository is exactly a file write into somebody else's tree. So the two cannot both be satisfied by one mechanism.

**The shape that follows is a hybrid, and it should be built as one deliberately rather than arrived at.** Stores get MCP surfaces, because only those can enforce the store's own invariants. Documentation gets a file-write allowance, because a markdown file has no invariants to enforce and nothing to enforce them with. That allowance is close to the path matching the earlier entry warned against - "a guard with no concept of a path cannot slide into being a path matcher" - and the warning is still right. The mitigation is that the allowance is narrow and stated: documentation only, never source, config, tests, scripts or fixtures.

**A consequence the seat has to carry, not the guard.** Documentation written into a repository inherits that repository's rules rather than the seat's own: its privacy hook, its language convention, its standard for what a repo document may say about a person. And a document the seat writes is one it must then not treat as evidence. It becomes what a session rooted there reads and believes, so a wrong one is wrong with authority - which is the failure recorded in the tier entry below, where two sessions independently read a superseded plan and reported against the person who had already superseded it.

**Still open**, and recorded so they are not decided by whichever code lands first: small scripts, where the distinction is durability rather than size, because one that answers a question today becomes a tool the moment it is rerun; a rendered overview of the person's own state; and repairing a store, which counts as building even when it is the seat's primary one.

## 2026-09-04 - A tier that gets routed around is not a tier

The orchestration model loses a level. The per-project orchestrator becomes the top seat below the captain, it is what "first mate" now means, and the cross-project tier above it goes away.

**The evidence is behavioural, not architectural.** In practice almost all work went through the direct widget, which bypasses the first mate entirely. A coordinator that is routed around does not become free: it still costs tokens on the turns it does take, and it costs attention as one more thing to hold in mind and switch between. Two seats already felt like too many to track; a third that mostly sat idle was paying rent on both.

**This was predicted and the mitigation did not hold.** The 2026-07-06 research pass warned that effective agent hierarchies cap at about two levels, that Claude Code's own Agent Teams disallows nesting, and that a cross-project coordinator only earns its coordination tax on *genuine* cross-project work. The resolution recorded then was to keep the tier but make it bookend-and-on-demand rather than a standing relay, with direct work in the project seats during the day. That is a design that depends on someone reaching for an optional thing. On-demand became never, which is the ordinary fate of optional layers and should have been read as the more likely outcome at the time.

**What absorbs the cross-project role.** The assistant seat, added two days earlier, is rooted in the meta-home, is standing, and is singular - structurally the same position the first mate occupied. Its own entry argued that it is *not* a first mate with a different manual, and that remains true about its permissions and its purpose. But it is where cross-project synthesis now lives, because it is the only seat that sits above every project and it is the one already being used that way.

**Dispatching through it is occasional by design, and the reason is measured rather than felt.** The default is to talk to a project seat directly; routing through the standing seat is a deliberate choice for the cases where a view across projects is worth paying for. A day of running it the other way produced the number: over 1588 turns, an orchestrating seat read 731 million cached context tokens, and by the end a single turn cost 670k. Because the whole context is re-read every turn, **the cost of an action in a standing seat is its context size, not the action** - relaying one instruction costs the same as reading a file or saying yes. Measured against typing the same instruction straight into the project seat, routing it through a standing seat cost roughly seven to eight times as many tokens at that point in the day, and about twice as many early on when the context was small. Cached reads are far cheaper per token than fresh input, so the money multiple is much smaller than the token multiple - but it grows monotonically for as long as the seat lives.

**Which corrects the diagnosis that started this.** The removed tier was not expensive because coordinating is expensive. It was expensive because it was *durable*, and this model already says so: project seats are ephemeral per assignment, the cross-project seat was a durable named role. A standing seat earns its cost when what it holds is irreplaceable - the person's own goals, the people around them, what was decided and why. It does not earn its cost when what it holds is a work queue, because a task board holds that better and can be re-read for nothing. Keeping one standing seat is therefore consistent with removing the tier, provided the standing seat is the one holding the irreplaceable half.

**Depends on somewhere else for the clock.** The standing seat must not hold recurring supervision - a watch loop with a timer makes the one seat the person converses with unavailable at moments the timer picks. That belongs in a separate short-lived seat that writes what it knows to the board as it goes, so it can be discarded and restarted for nothing. That seat is not yet a Helm kind, and it is a precondition for this arrangement rather than a follow-up: without it, merging the roles makes an occasional annoyance permanent and architectural.

**What changes:**

1. **The project seat is the first mate.** One per project, opened against a specific repo. Three things are being merged and not two: the **name** comes from the tier above, the **permissions** from the tier below, and the **identity** from neither yet. Identity is the hard one and the build breaks on it: a mate is `mate_<uuid>` with a slot, a pooled name, a persona and a retire path, while a project node is `sm_<hash>` minted from dispatcher plus path and has none of those. Pick "mate" and a seat that dispatches crew mints a project node underneath itself for its own project, which is the removed tier returning as a row. Pick "project node" and the slots, names, personas and retire path go. The permissions half is the easy half and stating it first makes the merge look settled when it is not.
2. **The direct widget goes.** Its whole population becomes named project seats, so there is one way to have a working session instead of three overlapping ones.
3. **Slots stay unlimited and the board degrades as they multiply, on purpose.** Two is the intended working number. The friction of a crowded board is the governor on concurrency, and it is deliberate rather than an unpolished corner: the real cost of another parallel session is attention, and a UI that hides that cost encourages exactly the thing being guarded against. Anyone tempted to tidy this away should read this paragraph first.
4. **There is one seat kind, and everything is a widget. What a seat IS comes from where it is rooted plus which persona it carries.** A seat rooted in a project repo is an ordinary project seat. A seat rooted in the meta-home *and* carrying the assistant persona is a standing assistant. This removes the separate assistant kind introduced two days earlier, along with its deliberate singularity, its slotlessness and its fixed name - a name was always wrong for it, because seat names are drawn at random from a themed pool and change on every retire.

**A trap inside the identity question, found while building the re-parenting and worth stating before someone "fixes" it.** When a seat is retired and its subtree moves to the successor, only the child's `firstMateId` moves - **the child node's own id is not rehashed.** "Re-parent" sounds like re-keying and it is not: the node id is `secondMateId(dispatcher, project)`, and the binding that holds the `sessionId` jump-in resumes is stored against the old key, so rehashing strands it. Mutating the resolver to rehash produces exactly that, measured rather than argued: two nodes for one project and `sessionId` back as null, so jump-in breaks on every retire. The visible consequence of NOT rehashing is that two rows for one project remain reachable, because a run dispatched by the retired seat and one dispatched by its successor hash differently. Nothing is orphaned and nothing is lost, and the duplicate is collapsed later by removing the dispatcher from `secondMateId` - not by re-keying the nodes. Anyone who meets the duplicate before that lands will reach for the rehash; this paragraph is why they should not.

**Rooting ALONE is not the discriminator, and that is settled by an old scar rather than by preference.** The 2026-07-15 entry below records exactly that version being run and reverted: first-mate treatment was applied on `isMetaHomeRoot(cwd)` alone, and every ordinary chat kept in the meta-home was stripped of its MCP servers and mis-framed as a first mate. The fix then was root **and** a declared binding. Root plus persona is the same shape, one field over, so an ordinary chat in the meta-home carries no persona and stays an ordinary chat.

**Four consequences, and three of them are things that stop existing.**

*The separate seat kind goes, and takes a live bug with it.* Slot reindexing was corrected on the day of this entry by filtering to coordinator seats, because the standing seat's deliberate `slot: null` sorted as zero and silently ate a slot on every retire. That filter exists only to exclude a slotless kind. With one kind, an assistant holds a slot like anything else and the filter has nothing to exclude, so the fix becomes unnecessary rather than needing re-verification against a changed premise, and it should then be deleted deliberately rather than left as harmless-looking dead defence. **The ordering is not optional: the filter stops being needed only AFTER the separate kind is gone.** Deleting it while a slotless assistant still exists re-opens the measured bug, so it is the last step of the migration and not a tidy-up to be taken on the strength of this paragraph alone.

*Plural falls out rather than being built.* Several seats rooted in the meta-home carrying the assistant persona, and nothing to guard against. Worth noting that this reconstructs the removed tier's own plurality, which was one per life-domain: what made that arrangement expensive was durability and disuse, not the count, so several standing seats that all get used are not the failure being corrected.

*The supervision seat is a persona, not a kind.* The dependency stated above wants a short-lived seat holding recurring watches that writes what it knows to the board. That is a meta-home seat with a supervisor persona and a shorter life, requiring no new tier, no new tool allowlist and no new manual injection point.

*Persona now survives respawn, and that is a change of default rather than an option.* `retireAndRespawn` takes `keepPersona` and it defaulted to false. A chosen persona carries to the successor seat. This is no longer only a convenience: with persona deciding what a seat IS, dropping it on refresh returns a standing assistant as a plain coordinator with the wrong manual and the wrong tools, and it reads as the seat having forgotten itself rather than as a config default. The code already carries a comment about a persona that "evaporated on the next refresh"; the same trap now has the whole seat identity behind it. 
*Flipping that default was one boolean and it exposed two defects, one of which is structural.* Keeping had been written as the leading branch, so once it became the default an explicitly passed persona became unreachable unless the caller also said `keepPersona: false` - a deliberate switch would have been silently ignored. And more interestingly: **`isValidPersonaKey` returns true for `null`, because `null` is how this codebase says "no persona"**, so using it to answer "did the caller name one" swallowed every ordinary refresh and returned null while the keep flag sat there true. The ambiguity is structural rather than a typo: `null` means both *not specified* and *explicitly none*, and the two only diverge at the moment absence stops being the destructive answer - which is the moment this change created. Anything reaching for that predicate as a "did they choose" test will make the same mistake.

*One deliberate behaviour change went with it:* an unknown persona key now leaves the seat as it was rather than resetting it to plain. It still never becomes the unknown persona, which is what that rule was for. Under the seat model a typo must not strip a seat's identity.

*And it answers the question it was meant to leave open: flipping the default is not sufficient.* Keeping is still encoded as an **absence** - saying nothing means keep - which is exactly what failed twice in one afternoon, in code written by someone holding the whole design in mind. As seat creation multiplies the callers, the reflex `persona || null` on the same line turns "refresh this assistant" into "make it a coordinator" with nothing to notice. The shape to check, deliberately not built ahead of the seat model: one three-state argument instead of two coupled ones, where a named key switches, an explicit sentinel clears, and absence keeps - so "keep" stops being spelled the same way as "I forgot to say".

**The cross-project view is both a page and a seat, and the split is reading versus acting.** The 2026-08-16 direction said the tier's value "survives as a PAGE rather than as a session"; the first draft of this entry moved it to a standing seat. Both, and the measurement above says why the distinction is not a compromise. A page is read when opened and costs nothing per turn: it answers *where do things stand*, which is the everyday question and needs no judgement, so it should never cost a turn of a large context. A standing seat re-reads everything it holds on every turn and was measured at 670k by the end of a day: it answers *what should I do about this*, which needs the context it is expensive because of. So the page is the default surface and the seat is the exception, taken deliberately when the answer requires synthesis rather than a look. Building only the page loses the synthesis; building only the seat charges 670k to read a list.

**Six forks the decision did not cover, settled the same evening, one at a time.**

1. **Identity: the merged seat is a `mate_`.** That namespace carries a slot, a pooled name, a persona and a retire path, none of which have substitutes. The alternative carries one property - being derivable - which a seat with a store record no longer needs. The consequence is not a special case: `secondMateId`'s dispatcher parameter has exactly one caller left once the tier is gone, the auto lane, so it becomes `autoNodeId(projectPath)` and the collision the auto split fixed cannot recur, there being no second identity to collide with.
2. **`helm_report_up` points at the captain's queue, not at the standing seat.** The standing seat is not in the chain of command for a project's work - it holds goals and people, not a work queue. And escalation is the *common* event, so routing the most frequent thing through the seat that re-reads its whole context every turn is the pattern this whole entry is about. The 2026-09-02 entry already names "a mandatory router" as a thing that would be wrong to build, which is exactly what pointing it at the seat would build. `helm_create_second_mate` and `helm_relay_to_second_mate` are re-pointed and become the standing seat's alone: a project seat opening another project's seat is the removed tier growing back from below.
3. **Sessions held by the direct widget migrate mechanically**, and this falls out of (1) rather than needing a plan. Those rows already are one node per project, so each becomes a seat rooted at that project which adopts its session.
4. **Model: Opus is the default and the picker overrides it, changeable at any time.** The cheap tier disappears with the cross-project seat, so the running cost rises and that is accepted rather than unnoticed. "At any time" is a constraint and not a nicety: the override has to work mid-session, not only at creation.
5. **Flipping the `keepPersona` default is not sufficient, and the fix waits for the seat model.** Keep is still spelled as an *absence*, which failed twice in one afternoon in code written by someone holding the whole design. The shape to build later is one three-state argument - a named key switches, an explicit sentinel clears, absence keeps - so "keep" stops being spelled the same way as "I forgot to say". Not built ahead of the model it serves.
6. **The write budget stays at three, and "too tight" is treated as a measurement.** `SECOND_MATE_TURN_WRITE_BUDGET` exists to stop an orchestrator quietly doing its crew's work, and the merged seat is still an orchestrator - the job did not change, only the name. The failure it prevents is documented and expensive: a seat with full tools and a manual telling it to delegate once absorbed the work instead and burned millions of tokens, with every prose guardrail present and none of them holding. Nobody has counted how often the budget actually fires, so it is left alone and counted over a week of real use. A carve-out for documentation writes was considered and rejected: it would make the most authoritative file in a repo the one free action.

**And a seventh, raised by the picker rather than by the migration: persona is two axes, so it is two pickers.** Coordinator, Architect, Teacher, Red team and Researcher are *temperaments* - the code calls the field a temperament overlay - and they change how a seat behaves. Assistant changes what a seat *is*: its manual, its tools, its permissions, its position. One list holding both makes one entry mean something categorically different from the other five, and the question that exposes it is whether you can have an assistant that is also a Red team. As temperaments, obviously yes; as one list, you chose one instead of the other. So: root decides the kind, the kind decides the temperaments on offer, and **the offered list is derived from the root rather than filtered by special-casing one entry** - a special case is how the next root-dependent persona gets forgotten.

**This entry is a restatement, not a first record, and the correction is worth more than the entry.** Two sessions independently searched for a prior decision, independently concluded none existed, and independently reported that the app was heading the other way. Both were wrong. The direction was decided on 2026-08-16 - "scarcity moves down to project seats, and the first-mate tier is on notice" - with its reasoning, its rejected alternatives and its failure conditions, and it was refined on 2026-08-31. This paragraph originally said the decision had never been written down. It was in this file, eleven lines below where that sentence was added.

**Why two independent searches missed a recorded decision**, which is the part to keep. Both searched for *strip*, *collapse*, *flatten*, *remove a tier*, *two tiers* and their Swedish equivalents. The entry contains none of those words. It says the tier is "on notice" and that the move is to "let the tier go", and its title leads with *scarcity* because that is what the decision is actually about. Searching for a decision you can already describe is searching your own vocabulary for someone else's writing, and it returns nothing with the confidence of an exhaustive result. The cheap correction is to search by date and read the headings, which takes longer and does not lie.

**And the accompanying documents were not lying either.** `PLAN.md` and `docs/orchestration-model.md` still describe the four-tier model as the target, coherently and with argument, because nothing propagated the 2026-08-16 decision into them. A superseded document is more dangerous than a missing one: it answers confidently, and it is what a session rooted here reads first.
...
Its own header comment states the exact principle the captain was asking for - a sentence written when the task is TAKEN constrains the work, the same sentence written at handoff merely describes it.
...
No code closes that. What the field does is stop the paraphrase from *reading* as his stated ask: `assistant` renders as "My reading of the ask, not confirmed by the captain", and an unattributed or unrecognised source falls back to `assistant` rather than up to `captain`.
A paraphrase still clears the gate deliberately - the captain does not hand-write a sentence per task, and a gate nobody can clear gets worked around rather than satisfied.
...
The captain's report was "den flyttades till in progress men sen verkar inget hända? inte som jag kan följa iaf. Auto widgeten är fortfarande tillsynes tom."
...
Verified before theorizing - built test-second-mate-dispatch-tools.mjs (a second mate DOES get helm_dispatch, so not a capability regression), then read his ACTUAL second-mate transcripts (last 7 days): EVERY one ground its whole assignment inline (dinghy/skiff, up to 42 edits) with ZERO helm_dispatch calls - and the prompts were exactly the batch case ("work the Jot list", "take care of these N p0 tasks") the manual warns against.
...
The captain (task 18d4c9f4): "what is fleet spend? I don't know where the $25 comes from - I'm on a subscription, not pay-by-usage."
...
Follow-up to the 2026-07-15 revert (which restored the false-positive bias): the captain asked whether we can actually tell if a first mate expects input - "look for a ?, or even let Haiku do a quick analysis when uncertain" - and to check both Swedish AND English phrases.
...
The archive button saves a handoff first (the existing "Save handoff to HANDOFF.md + archive" path; the label was corrected from the stale "DECISIONS.md"), per the captain's "they should leave their handoffs too".
...
Presented three options (re-parent to successor / surface as orphans / tear down); the captain's intention was option 3 - retire = "I'm done with this whole track".
...
Not yet behaviorally smoke-tested: a faithful test dispatches real Autopilot crew runs (heavier + side-effectful) rather than the first mate's single create_second_mate call; offered to the captain.
...
The captain asked whether we should have hard GUARDS so a first mate can't run away doing work in its own seat (ad17e2e6); the captain approved layers 1+2 and delegated the layer-3 form to me.
Three layers:
...
File mutation and sub-agent fan-out are structurally impossible in the coordinator seat - the dinghy 23-Edit runaway can't recur.
...
The dinghy case was ~143 turns at ~12% of a 1M context window, so the ctx% gauge alone would never have flagged it - turn count catches what context% misses.
...
The captain hit this in practice - jumped into a fresh first mate and found the composer pre-filled with the previous thread's orchestration handoff, when his usual reason for retiring is to START SOMETHING NEW, not continue.
...
The captain flagged that this collapses the orchestration tiers - the captain becomes first responder for every crew run, which is exactly what the tiers exist to avoid.
...
"Both, not either" (the captain's words): the dispatcher compiles/summarizes AND the individual runs stay openable for micro-analysis.
...
Alternatives rejected: (a) keep it flat on the Dashboard - rejected, it's the tier-collapse the captain objected to; (b) escalate-only, nothing under the mate - rejected, he wants the dispatcher to be the collection point AND retain drill-down, not just a filtered captain view.
...
The first mate, asked "I have a logo task for dinghy, jot and loom, fix all three", asked the captain to re-explain what the logo task was - instead of reading the tasks that were already in Jot, and instead of dispatching one per project.
...
The app's model and UI became fully nautical over the Fleet build - captain (the captain), first/second mates, crew, fleet, the anchor and compass icons.
...
The captain shared a popular blueprint for turning ONE Claude session into an autonomous org: a meta-prompt where the session role-plays an Orchestrator + First Mate + Second Mates, reprinting an orchestrator-log + backlog + agent-execution + final-output block (XML-tagged) in every response, invoking sub-agent "personas" for coding/critique inside the same context.
...
The captain's live-mic feedback after the kb-whisper-small Swedish-quality swap: "Swedish and 'auto' work BETTER than before, but the whole experience is SLOWER.
...
`onnx-community/kb-whisper-base-ONNX` (same KBLab Swedish training, roughly 1/4 the parameters) is documented as the next fallback in `voice.js` if `q8` still isn't fast enough on the captain's machine.
...
This proves the dtype string is valid for this repo and the pipeline loads/runs; it does NOT prove real-world Swedish transcription quality, latency, or mic-button responsiveness improved - that needs the captain's live microphone test, same limitation as every prior voice-input entry.

**What the captain needs to test:** hold the mic button and confirm it reacts instantly (no lag) even while a transcription is in flight; confirm a normal-length utterance transcribes noticeably faster than before; confirm Swedish (and "auto") transcription quality is still as good as the kb-whisper-small swap delivered; try continuous/rolling mode on a longer hold and confirm live partials keep up reasonably instead of trailing far behind.

## 2026-07-04 — Practitioner research validates the direction; 5 mechanisms to adopt, 4 traps to avoid

**Decision:** Before committing to the Helm rebuild (the captain: "before we
...
be shaped WITH the captain (the mock is the UI target; these mechanisms are the
...
This is explicitly a DRAFT for the captain to react to - scoped to the core loop plumbing (show mock -> annotate -> get structured feedback out), NOT a polished product.
...
This is explicitly a DRAFT for the captain to react to - the point is to make the already-built orchestrator TESTABLE, not to finalize its interface.
...
- The final summary card states explicitly, in the UI, that the run did NOT push or merge and that all work lives in the isolated worktree + branch (shown by path) for the user to review/merge or discard by hand - so the captain knows where the work went.
...
- NOT exercised live (would need a real autonomous run): a successful `iteration`/`done` event mutating state through to the summary card. That path is the same handler/switch already proven for the `error` kind, and both render funcs are proven to render correctly - but the genuinely-successful end-to-end run is left for the captain's own test, as instructed.

**This is a first-pass DRAFT needing the captain's review.** The UX is genuinely open (a dedicated page was one of several equally-valid choices); no coach/escalation layer (Point 12); single concurrent run; no re-run-from-summary, no worktree-open-in-explorer shortcut, no per-model/effort selection in the form (the backend accepts them; the form omits them for v1 simplicity). Point 11 remains IN PROGRESS, not done.
...
persisting through a real click in the running app. Both need the captain's own live
...
**Context:** the captain reported, after live-testing the 2026-07-03 "Live time/
...
code.** The most plausible real explanation for the captain's report is that his
...
**Thinking-dot color + animation (the captain's second ask, same message):**
...
pointing elsewhere, not the actual rules. The captain has now said directly he
...
(`<your-claude-home>/CLAUDE.md` on this machine) instead of the
...
errors, Halyard's PIDs untouched). Actually clicking the links and
confirming Explorer opens on the right folder with the right file selected
still needs the captain's own live test - no way to observe a real Explorer window
from here.
...
via `Get-CimInstance` that only Helm's own 4 PIDs recycled and Halyard's
...
direction for new orchestrator UI work. The captain explicitly delegated steering
...
**Also resolved a real technical question the captain raised:** if Helm's own
...
the pre-existing Halyard instance's PIDs were untouched, consistent with
...
server, so the usual preview tooling doesn't apply; the captain will confirm
visually.
...
**Context:** the captain asked me to act as orchestrator and dispatch agents
...
section?" — need the captain's specifics first), gated-on-the-firstmate-decision
...
Self-correcting and authoritative: as the captain runs each model through Helm
...
**Decision:** Iterated the context gauge per the captain's feedback (referencing
...
The captain's current environment (his own Claude Code shows "/ 1.0M"). The gauge
...
**Decision:** the captain's ask (with a Claude-Desktop screenshot showing "Context
...
**Bug:** the captain noticed a new Helm window opening without the old one
...
whether either use case is even possible headlessly. The captain approved running
...
**Decision:** the captain: "checkmarken bör gå att ta bort också (?) eller?" Agreed
...
## 2026-07-02 — Boot-test restarts were silently killing Halyard
...
The captain reported "Appen stängdes, jag tror du och halyard slåss om samma
port." Investigated instead of guessing: no port conflict — Helm's source
has zero `listen()`/`createServer()` calls anywhere. The real cause: Halyard
(nw-halyard) runs unpackaged in dev mode via `electron .`, so its process
...
Verified live: ran it while Halyard's 4 processes were up — Helm
restarted cleanly and all 4 Halyard PIDs were untouched afterward. This is
...
**Decision:** the captain's Jot task "performance + token usage granskning av
...
1. **Version numbering** (`src/lib/version.js`) — same scheme as Skiff
   ...
   instead of growing forever). Skiff computes this at Vite build time;
...
4. **Queue next prompt** ("flika in" scenario 2 from the captain's clarification:
...
Still needs the captain's hands-on confirmation that the *feel* is finally right.
...
and the captain's own testing. This is real architecture surgery across several
already-verified code paths, not an evening feature — flagging for the captain's
...
only. **Needs the captain to verify once**: launch Helm normally (not through a
...
autonomously while the captain is away. Helm's existing "Remove from Helm"
...
**Decision (the captain, settled in conversation):** Helm's mental model is an
explicit four-tier hierarchy — captain (the captain) -> one cross-project **first
...
valuable: the persistent coordinator the captain actually wants (the "Helm chat"
concept came from exactly this instinct). The captain re-surfaced it as the
...
- *Pure ephemeral* (the 2026-07-03 framing taken literally): loses the
  first-mate continuity the captain wants; a fresh officer every time who only ever
...
first mate is on-demand (invoked for real cross-project synthesis) or is the captain
...
ephemeral workers + a 2-level depth cap. To confirm with the captain before building
...
- **Model per tier is by JUDGMENT, not hierarchy level** (the captain's correction to
...
The captain's attention upstream so he can stop reading diffs.
...
The captain cannot see or CORRECT a criterion before the work starts, which is the whole
...
The captain: "störst effort borde ligga på systemkritiska moment. security issues borde
...
than one you tick, and the argument is something the captain can disagree with. Absences are
...
The captain, twice, on the same feature: the auto-captain started, the board moved to
...
The Auto widget reuses the captain's card by the captain's own request, so the card takes an
...
**Considered and not done:** splitting one session's handoff across two topics (the captain's
...
The captain: "kan du även gå igenom de misstag du gjort och gap för att täppa till dem i vår
...
The captain asked whether the test suite costs anything: "testsviten körs inte av ai eller hur, det är skript så den använder inte massa tokens?"
...
The captain, reacting to a screenshot with three near-identical skill widgets: "Gör inte en widget per projekt, kommer växa och ser dåligt ut. Kanske istället en pill eller dropdown i en widget för att välja projekt man vill visa för."
...
- **Deliberately left unfixed, on the record:** the column is not centred (doing so means constraining the whole transcript including right-aligned user turns — a layout decision, not a one-liner; landed 3 commits later in `815b6b2`, see below), the body face was never changed to match the reference (only its weight — the sans-not-serif choice above was the captain's own call), nested lists still flatten, and a 600-line block costs ~96ms to colour.

## 2026-08-05 - The chat column narrows by capping the scroll's children, not the scroll itself

The captain, closing the one item the independent review above left open on purpose: "ja, jag vill testa. men det ska fortfarande vara vänster/höger ställt bara att chattfönstrets bredd minskar."
...
The captain: "review vyn renderar inte" — the review page rendered NOTHING (not one broken row, the whole page), shipped in 0.1.586.
...
## 2026-08-06 - A mate-bound session can outlive the ONLY index that pointed to it

The captain: "något skumt har hänt i helm. Oavsett vilken 1st mate jag går in i är historiken borta och det står att det är ett helm projekt" — jumping into either first mate opened a blank draft instead of resuming its real conversation.

Not a rendering bug: three separate reproductions against the real running app's own data (its `config.json`/`mates.json`, copied read-only) rendered the real transcript perfectly on current code. The gap was upstream of rendering entirely. `mates.json` still pointed the "Guybrush Threepwood" first mate at a real, intact, 61-line session on disk — but `getSessions()` had no way to find it: it merges two sources (the Desktop app's own rolling `local_*.json` index, and Helm's own `config.helmSessions` record for sessions the Desktop app never sees), and this session's id was in neither. `jumpIntoFirstMate` searches exactly that merged list, finds nothing, and falls back to "open a fresh draft" — which reads as "the history is gone" even though nothing was deleted.

**Ruled out first, with evidence, not assumption:** the captain had archived this exact session shortly before noticing the bug and asked directly whether that caused it. `applySessionArchive` only ever sets an `isArchived` flag or mirrors it into an EXISTING `helmSessions` entry — it has no code path that removes one, and the missing entry had never existed in the first place, archived or not.

**Actual root cause:** `recordHelmSession` (the write side of `config.helmSessions`) was called with `createIfAbsent: !resumeSessionId` — deliberately skipping indexing on a resumed turn, on the reasoning that "a resumed Desktop session isn't ours to index," i.e. trust the Desktop app to keep tracking it. That reasoning holds for an ordinary resumed chat. It does not hold once the session is bound to a mate: the Desktop app's index is a rolling window (it does not promise to remember a session forever), while a mate's binding in `mates.json` is meant to be permanent. A session can age out of the Desktop's own bookkeeping while Helm's own mate binding still points at it, and until this fix Helm had no record of its own to fall back on.

**The fix:** `createIfAbsent` is now forced `true` whenever the current turn is bound to a first or second mate, on both the paths that record a fresh session id (`session:start`'s onEvent, the second-mate relay's onEvent) — regardless of whether it's a fresh launch or a resume. A third, defensive call was added to the `mates:bindSession` IPC handler itself (the renderer's own bind-on-session path, used when the server-side path is skipped by a pane reassignment) so a mate-bound session is indexed on whichever path actually runs. `recordHelmSession` upserts, so calling it twice for the same id is harmless.

**Verified against the real bug, not a synthetic case:** reproduced the exact failure by copying the captain's real `config.json`/`mates.json` into an isolated harness run and driving `jumpIntoFirstMate` on the real slot-0 mate — confirmed it fell back to a blank draft before the fix and resumed the real 20-turn conversation after it. His live `config.json` was patched directly (with a backup taken first) to restore the missing entry immediately, ahead of the next release.
**Not covered by an automated test:** the new logic is a one-line boolean condition inside `main.js`'s `session:start` handler, which spawns a real `claude` process — there is no token-free way to exercise it through the existing harness without mocking the launcher, which felt like more machinery than a one-line condition warrants. The fast suite (66/66) shows no regression from the change; the real-data reproduction above is the actual evidence the fix works.

## 2026-08-06 - Chat formatting (c6094e4f), a third round of measured feedback

The captain, after living with the centred column from the previous round: the column is a bit too narrow, the composer should not go edge-to-edge either, and his own (user) turn text looks thin against the orange fill.

- **Chat column: 860px -> 1040px** ("bredden kanske aningen for liten, kan vi oka den kanske 20% eller sa" - 1040 is +21%). `test-chat-markdown.mjs` asserts relatively (narrower than the pane, centred), so it needed no change to keep passing.
- **Composer gets its own cap, 1320px, wider than the chat column on purpose** ("Inte fullt lika smal som resten men absolut inte anda ut till kanterna") - a separate value, not `--chat-column` reused, since "not as narrow" means intentionally wider, not equal.
- **User bubble font-weight: 450 -> 520.** Measured the actual contrast first rather than guessing which of "weight" or "the orange itself" (the captain's own two hypotheses) was real: 5.57:1 on the active (dark) theme, which clears WCAG comfortably. So this is a perceived-weight fix, not a contrast one - a solid saturated fill reads thinner at a given numeric weight than the assistant's bordered card does at the same weight. All three values verified by measuring the live rendered DOM (`getBoundingClientRect`/`getComputedStyle` against the real pane), not by eyeballing a screenshot.

## 2026-08-06 - Analysis: a hairline between the project picker and its skills (07658c1a)

The captain: "kan vi lägga en avskiljare mellan projekt pills och skills." The picker already had an 8px/10px margin around it, which turned out not to be the fix - both rows use the identical `.dash-chip` pill shape, so more air read as one long row of pills, not two grouped things. A `border-bottom: 1px solid var(--border)` (the same token every other divider in the app uses) does what the spacing alone could not: give the eye an actual edge to stop at.

## 2026-08-06 - The review diff goes side by side (c3dfbb42)

The captain, task c3dfbb42, standing ask since 2026-08-05: "Kan vi få en side by side diff istället som i vanliga git klienter" - the diff viewer only ever rendered the raw unified patch as coloured text in one column.

**The parsing is the actual work; the two columns are just where it's spent.** A unified diff interleaves deletions and additions with no structural pairing - `parseUnifiedDiffFiles` (renderer.js) walks each hunk's lines and uses the same rule every git GUI does: a run of consecutive `-` lines and the `-`/`+` lines immediately following pair up positionally (row 1 with row 1, row 2 with row 2, ...); whichever side has more lines gets the extra rows with a blank filler cell on the OTHER side, rather than the extra lines silently vanishing or getting glued onto a mismatched row. A context line (leading space) is never pulled into a pending pair - it always closes out whatever run came before it and gets its own row, identical on both sides.

**Two real defects found only by looking at a screenshot, not by reading the code:** the line-number gutter was a fixed `3em`, which wrapped a 4-digit line number onto two visual lines ("5499" rendered as "54" over "99") the first time this ran against a file with 5000+ lines - `style.css` itself, ironically. Fixed by sizing the gutter columns `auto` instead of guessing one width for every file, plus `white-space: nowrap` on the gutter cells as a backstop. The second file diff was tested with a small synthetic file it wasn't caught there because the em width happened to fit 1-2 digit numbers - this is the same lesson task c6094e4f's independent review already wrote down twice: reasoning about a rendered layout is not the same as looking at it.
...
## 2026-08-07 - The diff viewer gets a changed-files column

c3dfbb42, one more round: "borde finnas en till kolumn där man kan se ändrade filer och väljer fil att se diff för där." `.docv-modal` (already shared by the plain doc viewer and the diff viewer) gained a `.docv-row` flex row holding an optional `.docv-filelist` sidebar beside `.docv-body`, so the head stays pinned while EITHER side scrolls on its own. `openDocViewer` explicitly hides/clears the sidebar (a single `.md` file has nothing to list); `openDiffViewer` populates it from `parseUnifiedDiffFiles`' own file blocks, deduped and in first-appearance order.
...
**The board-state half of the widget is gone.** Total in review, needs-judgment, ready-to-stamp, unconfirmed, criticality bars - all removed. That was always the Review page's own job (same `reviewQueueTally`); the captain's ask was specifically for what HE does, not the board's current shape, so keeping both in one block was answering a question he didn't ask alongside the one he did.
...
## 2026-08-07 - A handoff commits itself; an independent reviewer's write no longer stalls on a question nobody can answer

Two bugs the captain found by using the app, not by review.

**HANDOFF.md sat untracked for weeks.** It had been written once and never committed - `git status --porcelain` therefore reported the Helm repo dirty on every single check, permanently, unrelated to whatever was actually being tested. That is the ACTUAL explanation behind "testerna säger nästan alltid ran on uncommitted changes" (`reviewRecords.js`'s `headDirty` gate, `git status --porcelain` scoped to the whole repo, not the files under test). Fixed two ways: committed the stray file (`11cd249`) to clear the immediate symptom, and - the captain: "ja, en handoff borde commitas direkt" - `context:saveHandoff` now runs `git add -- HANDOFF.md` + `git commit` right after writing it. Staged by NAME, never `-A`: the ordinary edit-then-test-then-commit flow usually leaves something else uncommitted at the moment a handoff saves, and sweeping that up too would silently commit work that was not ready. Best-effort and silent on failure (not every project folder is a git repo). New test in `test-handoff-file.mjs` proves the scoping specifically - an unrelated dirty file survives uncommitted right next to a handoff that gets committed.

**The independent reviewer's own verdict write triggered a permission prompt nobody could answer.** Its brief (unchanged, written 2026-08-05) always told it to write outside its own project directory, into the meta-home's `.helm/reviews/` - which the ordinary permission gate correctly treats as more sensitive than an in-project write. The dispatch is a headless `claude -p` launch with admittedly no live channel to answer a prompt (a limitation already documented elsewhere in this file), so the question just sat there - surfaced to the captain as literal chat text ("Would you grant permission to write the verdict file?") with no way to click yes. Root cause was `session:start`'s IPC handler: a plain (non-mate) launch had no way to receive a caller-supplied `allowedTools` at all - the variable existed only to be filled in by the first-mate/second-mate branches, so anything the renderer passed for an ordinary session was silently discarded.

Fixed at both ends: `session:start` now seeds `allowedTools` from the caller before the mate-specific branches (which still override it, since their own scoping is more specific), and `openIndependentReview`'s dispatch passes `allowedTools: ["Write(<exact verdict path>)"]` - not a broader permission mode. Deliberately narrow: `bypassPermissions` would have fixed the stall too, but it would also have silently dropped the brief's own instruction ("Do not change anything else. This is a review.") from being technically enforced to merely requested - a misbehaving reviewer could edit the real project with no gate at all. Scoping to exactly the one file it needs keeps that guarantee intact.

The args-building was pulled out of the confirm-button's callback into `independentReviewSessionArgs`, a pure function, specifically so a test could pin its exact output - `window.helm` is a contextBridge object and silently cannot be reassigned/stubbed from a renderer test (confirmed by trying it first). `test-reviewer-dispatch.mjs` now calls the real function directly and asserts the exact `allowedTools` entry, with no IPC call and no token spent.

## 2026-08-07 - Review checks run against a pinned commit; the verdict becomes something you can read and act on

Task 76790f23, a third round of feedback, three parts.

**1. Check runs are bound to the review record's own commit.** The captain: "Bind varje review till en commit för att förhindra 'Ran on uncommited changes' när man kör run checks." The deeper cause behind the recurring stale stamp (the untracked HANDOFF.md was one instance; this is the general fix): `reviews:runChecks` ran each check straight in `rec.projectPath`, so whatever was sitting uncommitted there *at that moment* - usually unrelated work-in-progress on the next task - stamped every result `headDirty: true`, regardless of whether the code under review was itself committed. Now the whole run happens in a fresh **detached** worktree checked out at the record's own commit (`createDetachedWorktree` in worktree.js - detached, not a new branch like `createWorktree`, so it litters no branch list and is pinned to a committed sha rather than "current state"), and `recordCheckRun` takes a `pinnedHead` so it stamps the commit actually verified instead of re-querying the live dirty tree. A monorepo check's `cwd` offset is re-rooted into the worktree. All best-effort: no commits, not a git repo, or a worktree that won't create → falls back to running in place, because a check in a possibly-dirty tree still beats no check. The worktree is removed after (force + ignoreBookkeeping, since it carries the junctioned node_modules).

**2. The verdict is readable, and you can get a second opinion.** The captain: "jag vill kunna se verdict lättläst ... och om jag inte håller med eller vill ha second opinion vill jag ha möjlighet att skicka verdict vidare till en till granskare." The inline verdict now renders as markdown via the same safe `renderMarkdownInto` chat replies use (it builds DOM from the text, never innerHTML - an agent-authored verdict is exactly the same trust level as a model's chat reply, so the earlier plain-text caution didn't actually hold), with the markdown element styling broadened from `.turn-bubble` to `:is(.turn-bubble, .rev-independent-note)` so headings/code/lists actually look like themselves in both places. Three actions on the verdict: **View verdict** (full, resizable doc viewer), **Second opinion** (a FRESH reviewer told what the first found, asked to reach its own conclusion first then say whether it agrees - not a resume, the first session has exited; the prior verdict goes in *after* the instructions and is framed as a claim to test, or asking twice is worthless), and **Open a fix session** (part 3).

**3. Getting the verdict to whoever fixes it.** The captain: "Hur ger man tillbaka den oberoende granskarens feedback till [den] som gjorde featuren?" **Open a fix session** seeds a fresh draft in the project with the verdict as the brief - framed as a *claim to reproduce first*, not a work order (a reviewer can be wrong, and a session that assumes otherwise "fixes" things that were never broken - the same posture the review brief takes toward the author, pointed the other way). Deliberately does NOT auto-send: the verdict can be the very thing in dispute, so it's left in the composer to read and edit before a token is spent. Distinct from "Send back", which is the board-status decision, not the start of the actual fix.

Tests: new `test-review-checks-worktree.mjs` (a genuinely dirty repo, asserts the run stamps `headDirty:false` bound to the record's commit, the unrelated dirty file survives untouched, and no worktree is left behind); `test-reviewer-dispatch.mjs` extended for the three verdict actions and the second-opinion brief's contents. Fast suite 66/66.

## 2026-08-09 - Helm can re-authenticate, and the model picker reaches older models

Two tickets the captain filed after hitting them in use.

**Auth (3218cdd4): "Failed to authenticate: OAuth session expired and could not be refreshed."** Helm wraps the real `claude` CLI and inherits its login; when the CLI's OAuth token expires and can't refresh, every headless `claude -p` launch fails and there was no in-app way to fix it - you had to drop to a terminal. I agreed Helm needs an auth affordance but explicitly did NOT build one: reimplementing OAuth (token store, refresh, callback server) duplicates what the CLI already does and is exactly the kind of thing Helm's "wrap the real CLI" rule exists to prevent. Instead two IPC handlers DRIVE the CLI's own `claude auth` subcommands - `auth:status` (`claude auth status --json`) and `auth:login` (spawns the real `claude auth login --claudeai` browser flow, streams the URL as a fallback, re-reads status on exit rather than trusting the exit code). Helm reads the same credential store the CLI writes, so one successful login fixes every subsequent launch. Surfaced two ways (the captain picked the fuller option): a persistent Account group in Settings showing who's signed in (email/org/plan) with a Sign in / Re-sign in button, AND a one-click "Sign in" shortcut on the notice that fires when a session actually errors with an auth-shaped message (`maybeSurfaceAuthError`, matched loosely enough to catch the CLI's wording variants without firing on unrelated errors containing "auth"). Single-flight guard on login (a second concurrent flow would fight over the callback port); 3-minute ceiling so an abandoned login can't wedge the guard forever.

**Model picker (cf96055c): "Kunna välja tidigare modeller ... helst i en submeny för att inte plottra."** `dropdownPill` gained submenu support (an option can be `{ label, submenu: [...] }`; the context menu already rendered submenus, and the pill's own label lookup now flattens both levels so a submenu pick still names itself). One shared `MODEL_MENU_OPTIONS` now backs the composer, goal, and routine pills - current-generation models (Sonnet 5, Opus 5, Opus 4.8, Haiku 4.5) at the top, older ones (Opus 4.7/4.6/4.5, Sonnet 4.6/4.5, Fable 5) behind "More models", mirroring the desktop app. Opus 4.8 is deliberately kept at the TOP level, not buried: it's the second-mate default and the one older model in active use here.

**Why 4.8 stays first-class - researched, not assumed.** The captain's report that "opus 5 inte håller sig inom ramarna och folk tycker opus 4.8 performar bättre" checks out and is Anthropic's OWN documented behaviour, not just community noise: Opus 5 interprets instructions differently from 4.8 (expands scope, over-verifies, runs longer/more verbose), and Anthropic ships explicit migration guidance saying prompts written for 4.8 should be updated for 5. Every Helm prompt (coordinator, reviewer, second mate) was written and tuned against Sonnet 5 / Opus 4.8, so under Opus 5 they'd drift unless rewritten. The one place Helm defaults to Opus 5 is the independent-reviewer recommendation (`reviewerModel.js`, critical items); the captain's call was to KEEP that default and simply make 4.8/older easy to pick, not flip it - so no `reviewerModel.js` change, only the picker. Sources: Anthropic's Opus 5 news / migration guide / Opus 5 prompting guidance.

Model ids are not guessed - they were extracted from the strings bundled inside the installed `claude.exe`, so every id in the menu is one the CLI actually resolves (a pick can't become a control that errors on an unknown model). `test-model-menu-and-auth.mjs` pins the submenu structure, that a submenu pick sets both value and readable label, that every id is in the CLI's known set, and the auth status/phrasing/error-surfacing behaviour (driving the real `claude auth status`, but never `claude auth login` - that opens a browser). Fast suite 66/66.

## 2026-08-09 - Four in-progress cards: token-reset time, captain project, review focus, and the state of interactive mocks

Four cards the captain put in progress.

**Schedule-on-quota-reset gave the wrong time (5143316e: "ger helt fel tid").** `quotaResetFireAt` picked the soonest window whose LAST-KNOWN `resetsAt` was still in the future. But the rate-limit windows recur: a five-hour window that ended at T ends again at T+5h, T+10h... The CLI only ever reports the end of the last-observed window, so once a short window's stored end had passed, that reading was discarded and the schedule fell through to whatever window still had a future stamp - the seven-day one. Confirmed in his real queue: a "send when quota resets" entry created 2026-08-05 was labelled "at quota reset (~86h)", pinned to the weekly boundary instead of the next few-hour reset. Fix: an elapsed reading of a window whose period we can parse (`windowPeriodMs` reads "<count>_<unit>[_<model>]", e.g. five_hour / seven_day / seven_day_opus) is rolled forward to its next boundary; a window whose period we can't parse still yields nothing when elapsed (no guessing from a truly dead reading - the pre-fix rule for that case is kept).

**The captain view now says which project each session runs against (0c4494ce).** A fleet row's name is the session TOPIC, which only hints at the repo; the actual folder was invisible. Each row now shows the folder it's rooted in (from `sess.cwd` / the node's `projectPath`), with the full path on hover so two same-named folders read apart. Shown for every fleet row, not only the captain's.

**The work/private toggle is back - on Review, where it filters WORK (0ca1f3d3).** This looks like a reversal of 427c73a ("Remove the Focus track ... den gör inget vettigt nu"), and deliberately isn't. The old toggle filtered the Focus/Goals list - a second place to write down intentions that duplicated the Jot board, which is why it did nothing useful. This one filters the Review queue: the finished tasks he actually looks at, sorted by whether they're work or private. The domain comes straight from each Jot category's own W/P classification (all 14 of his boards are already classified), threaded through `reviewTasks` -> `buildReviewQueue` onto each row, and surfaced as a segmented All/Work/Private control at the head of the filter bar. Same anti-trap rules as the project chips: chip counts come from the pre-domain set so switching focus never hides the control that switches it back, and picking a domain clears the project chip so an empty view can't strand the page.

**Interactive mocks (ed305bc2: "hur blev det med ... som kun chens lavish") - already built and now verified end to end.** The Lavish-style annotate-feedback loop is shipped: a dedicated Plan page loads a mockup (file or pasted HTML, with recents), renders it in a sandboxed null-origin iframe with the annotation SDK injected (lifted/trimmed from Kun Chen's lavish-axi, MIT - see `lib/lavishSdk.js`), lets you click an element and type feedback ON it, and turns the collected annotations into one TOON-encoded, agent-ready block via "Copy feedback" / "Send to composer". A session that writes a `*mockup*.html` file raises an "Open in Plan" banner straight into the annotator. PLAN.md files further investment under "parked/optional" - the machinery exists; what remains is using it in daily practice, not building. Found and fixed in passing: the two Lavish E2E tests (`test-open-mockup-in-plan`, `test-mockup-banner`) had defaulted their fixture to a hardcoded path inside a since-deleted session scratchpad, so the file-open half failed for everyone; each now writes its own temp mockup. With that, both pass end to end (banner + open + sandboxed iframe render, 0 console errors), which is the actual evidence the feature works.

### Ship-review follow-up (same day): the review focus was shrinking the global badge

An independent ship-review pass on the four commits above found one real, probe-confirmed bug in the Review work/private focus.
The subnav review badge is a GLOBAL attention signal - it already passes `ignoreProjectFilter` so a project chip can't shrink it - but the new domain clause in `visibleReviewRows` was applied unconditionally.
Picking "Work" therefore dropped the private review items from the badge and kept it shrunk until restart, with nothing on screen saying so: the exact under-flagging bug (daa4245f) `ignoreProjectFilter` was built to prevent, reintroduced on a new axis.
...
## 2026-08-09 - Strategy for noticing a new Claude model release (Jot card, unlabeled id)

The captain's card: "om t.ex opus 5.1 släpps fångar inte helm det" - model ids live hardcoded in three places (`REVIEWER_MODELS` in reviewerModel.js, `MODEL_MENU_OPTIONS` in renderer.js, and ad-hoc strings in suggest.js), each added by hand when he noticed a new release. Nothing tells him a new one exists.

**Why not the obvious options.** `GET /v1/models` needs an `ANTHROPIC_API_KEY` - Helm has none; every session shells out to the `claude` CLI on its own OAuth session (launcher.js). A docs-page scrape is fragile HTML-diffing and needs a network call on every boot for something that changes on the order of months. An npm-version diff of `@anthropic-ai/claude-code` only proves the CLI changed, not that a model did.

**What it does instead:** `src/lib/modelFreshness.js` reads the model ids the installed `claude` binary itself already recognizes - literally the same thing the captain was doing by hand ("verified against the ids bundled in claude.exe, not guessed", the comment above `MODEL_MENU_OPTIONS`). It finds the binary via `where`/`which claude` (picks the largest candidate - the real compiled binary dwarfs the `.cmd`/shim wrappers on PATH), reads it as latin1 text, and regexes out tokens shaped like `claude-<family>-<digits>(-<digits>)*` with lookaround boundaries so `-v1`/`-fast`/`.md`/typo-dotted noise baked into the binary never matches.

To avoid flagging every legacy back-compat alias the CLI still carries (opus 4, 4.1, sonnet 4-base, dated snapshot pins like `-20251101`) on every single run, it compares per-family VERSION TUPLES against the highest one in the new `src/lib/models.js` (`KNOWN_MODEL_IDS` - a union of what reviewerModel.js and renderer.js already list, not a replacement for either; the renderer is a classic script and can't `require` it, same constraint as `WORKING_LIFECYCLE_STATES`). Only a version strictly ABOVE the tracked max per family counts as new. Verified against the real installed binary both ways: the current CLI reports nothing new, and a scratch run with the known-max lowered back to 4-8/4-6 correctly surfaced exactly `claude-opus-5` and `claude-sonnet-5` as "new" - nothing else.

Wired exactly like the existing stale-build pill (`runStaleBuildCheck`/`build:staleUpdate`): `runModelFreshnessCheck` in main.js runs once at boot and once a day, pushes `models:freshnessUpdate` only when the new-id set changes, and the renderer shows a small header pill (mirrors `#staleBuildPill`) naming the id(s) with a tooltip pointing at `src/lib/models.js` as the one file to edit. Deliberately no auto-anything: adding a model to the picker or a reviewer tier is a judgment call (where it slots, what effort/criticality tier), not something to automate.

## 2026-08-09 (batch 4) - The classic dashboard is gone; widgets are the only layout

Task 337895ce ("widget vyn är nu testad och vi borde ta bort classic layout").
Done in two commits so the behaviour change could be verified before any code was deleted - the DECISIONS lesson from the Focus removal (a deletion that "still parses" is not one that "still works"; only booting and rendering every view catches it).

Step 1 (behavioural): the three gates that chose between the widget grid and the classic section stack now always take the widget path (renderDashboardPage, fillDashboardSections' tick repaint, repaintDashboard). The "Classic layout" / "Widget layout" toggle buttons are gone; config's dashboardWidgets.enabled defaults true and is vestigial for layout choice. Verified by booting the app and rendering all eight views: every one renders, the Dashboard is the widget grid, no classic slots, no toggle, 0 console errors.

Step 2 (deletion): removed the now-unreachable classic-only code, guided by a mapping pass that classified each helper as classic-only vs shared-with-widgets (the widgets "lift the body out of" some classic builders, so it is not a clean excision). Deleted: renderDashboardPage's classic body, fillDashboardSections' classic tail, dashboardFleetSection + fleetColumnEl, dashboardDriftSection + driftSectionEl, dashboardNewSessionSection + dashChipEl + dashAutoContextStripEl, dashboardOnboardingBlock, the three classic fingerprints + dashSectionFingerprints, setWidgetDashboardEnabled, and the already-dead `w-goals` default-layout entry (it rendered as "Unknown widget goals"). Kept (verified shared, must not delete): fillDashboardSections' widget branch, dashboardQueueSection, dashboardInMotionRows, fleetMateCardEl, fleetDirectCardEl, fleetMateReportRollupEl, buildFleetModel, newSessionFolderMenuItems, the drift helpers. ~460 lines removed.

Parity, decided explicitly: onboarding was classic-only and its copy described the classic sections ("New session", "goal below") - dropped, not ported, because it would be wrong on the widget dashboard. The "+ Session" starter survives (Captain widget's own button); only the project-chip grid presentation was dropped. Proposed second mates still surface (fleetSecondMateEl's "proposed - engage" branch under the first-mate widget), so the classic proposals banner going away is a presentation change, not a lost capability.

Test fallout (the removal's real cost): the classic surfaces had E2E tests. Deleted the ones whose whole subject was removed (test-fleet-view, test-dashboard-onboarding, test-dashboard-chip-select, test-fleet-proposals - the last verified: proposal ENGAGE still works via the widget branch, only the banner test was classic). Retargeted the ones whose subject survives to render the widget-equivalent into a probe container instead of the classic slot (test-direct-report-rollup, test-docs-staleness, test-fleet-ui-fixes, test-ship-review-fixes, test-auto-widget-visibility's Direct-surface count 2 -> 1). Fast suite 66/66.

## 2026-08-09 - Full-suite fallout from the classic-dashboard removal exposed three pre-existing, unrelated bugs

Running the *full* E2E suite (not just fast) after the classic-dashboard removal surfaced failures the removal itself didn't cause - fixed at the root instead of muted, since each was a real latent bug:

- **needs-you/archive/proposal surfaces read raw `status` instead of the FSM's `lifecycleState`** - a regression from the status-as-FSM work (Epic f3d096fa): fixtures that set only `status` (not `lifecycleState`) had gone silently invisible to those filters. Fixed to read the FSM field, matching how the rest of the app already treats status post-FSM.
- **Orchestrator detection had moved from cwd-based to mate-binding-based** (the "+ New orchestrator session" button was removed earlier, commit 4ba33f3) without updating the tests that still assumed cwd-based detection - retargeted to assert against the binding.
- **`test-relay-delivery` was missing the shared `--live` token-spend gate** and so ran for real (spending tokens, timing out) on a plain full-suite run instead of only under `--live`; fixed-timeout races elsewhere in the same sweep were replaced with polling to stop being flaky under load.

No design decision here beyond "fix the root cause, don't skip/mute" - included because it closes real gaps (FSM-blind filters, stale orchestrator-detection tests, an ungated live test) that would otherwise resurface as confusing failures later.
...
Task: show a "shipped in vX" chip on a review record by resolving the earliest release tag that contains all of a task's commits (`shippedVersionForCommits()` in reviewDiff.js, `git tag --contains`). First landed, then two bugs surfaced same-day from an independent review pass and were fixed as a follow-up rather than patched around:

1. It assumed `commits[].at(-1)` was the newest commit, but `resolveTaskCommits`'s log-fallback path returns commits newest-first while the record-sourced path returns oldest-first - so "the last commit" meant different things depending on which path populated the array. Fixed by computing per-commit earliest-containing-tag, then taking the latest across all commits (null - unreleased - if any single commit has no containing tag), which is correct regardless of array order. Rejected: normalizing array order at the source, since two call sites would need to agree on a convention forever rather than the consumer just not caring about order.
2. The IPC handler cached `null` for a whole run and only re-fetched git tags once per run, so a release cut mid-session (a real, common case - the captain ships while the app is open) never surfaced its chip without an app restart. Fixed by caching only positive (released) results and throttling the tag fetch per-project to 60s instead of once-per-run.

## 2026-08-09 - Review rooting made project-agnostic by trusting Jot's own `Category.repoPath`

Reviews had only ever "worked properly" for the helm project itself. The review pipeline populated a row's `repoPath` via a fuzzy category-name-to-cwd guess (originally added to backfill records that predated any explicit field), and the review display filter hides any row with no resolvable `repoPath` - so every other project's reviews were silently dropped unless the guess happened to match. Jot already stores an explicit `Category.repoPath` per category; the pipeline just wasn't reading it. Fixed by threading the explicit path through `jot.js reviewTasks` -> `reviewRecords.js buildReviewQueue` -> `buildReviewsPayload`, preferring it and falling back to the old fuzzy guess only when it's absent (keeps old records working). Rejected: hand-fixing each project's stored repoPath, since Jot already has the right data and the fix is one field to trust rather than a one-time cleanup that regresses the next time a new project category is added.

## 2026-08-09 - Transcript duplication fixed by making a completed turn's file-reload authoritative, not merged

Live streaming rendered assistant-text blocks as they arrived, but not tool-call turns. On completion the transcript file was reloaded and merged in - bringing its own copy of the just-finished turn (grouped as "Used N tools") alongside the blocks already streamed for it, which showed up as literal duplicate bubbles whenever a turn was tool-heavy enough to fall outside the existing 60-turn tail-match dedup window. Fixed by having `loadTranscriptInto({ authoritative: true })` treat the freshly-reloaded file as the sole source of truth on completion, discarding the pending streamed buffer instead of trying to merge/dedup it. Rejected: widening or retuning the tail-match window, since that stays probabilistic (some turn size will always exceed whatever window is chosen) where authoritative replacement is exact. Same pass also gave each pane's busy spinner a backstop clear-on-"closed", so a process that exits without ever emitting a done/error event can no longer leave "Working…" stuck until an app restart.

## 2026-08-10 - Version bumped to 0.2.0, deliberately not 1.0

A milestone marker, not a stability claim - the patch counter had reached 627 since the 0.1 baseline, with the review pipeline, autopilot/second mates, the widget dashboard, model-freshness checking, and the transcript work all having landed since. Explicitly not 1.0: core surfaces (status as an FSM, the dashboard, review) are still being actively reshaped, which is 0.x-shaped behaviour, and with no external consumer, a major-version "breaking change" contract would only bind the captain to himself for no benefit - deferred until those surfaces settle. (Supersedes the "Installer past 0.1.586" note in PLAN.md's status header, now stale.)

## 2026-08-11 - Fleet crew detection retargeted at the CLI's actual tool name ("Agent", not "Task")

`subAgents.js` matched `tool_use` blocks named `Task` to render live crew rows in the Fleet tree; the CLI had renamed that tool to `Agent`, so no live sub-agent ever showed up, and `FIRST_MATE_DISALLOWED_TOOLS` (which named the same stale string) silently stopped blocking a first mate from fanning out its own sub-agents. Fixed by retargeting both to `Agent`. No alternative considered - this is a name the CLI owns, not a design choice Helm made; logged because a disallow-list keyed to a stale name fails OPEN (guard silently becomes a no-op) rather than loud, which is the dangerous direction for that kind of list to fail in.

## 2026-08-11 - A queued follow-up prompt is tracked per session, not per pane

`pane.queuedPrompt` lived only on the transient pane object, which `openSessionInPane` discards and rebuilds on every navigation - so "queue a prompt, step away, it fires when the run finishes" broke on the "step away" half, the exact case the feature exists for. Fixed by moving it into a session-keyed map alongside `runningSessions`, restored on reopen, and fired immediately if the run already finished while the pane was closed. Rejected: persisting the queued prompt onto the pane in a way that survives rebuild - session-keyed state is the pattern `runningSessions` already established for exactly this "outlives the pane" requirement, so this just extends it rather than inventing a second mechanism.

## 2026-08-11 - A "done" event's terminal branches all reload the transcript and clear the pending-turn buffer

Stop / CLI-failure / no-result terminal branches of the "done" handler never reloaded the transcript or cleared `pendingTurnsBySession`, so a streamed turn that didn't match the file's tail window on a later reload stayed in that buffer forever and got re-appended every time the session reopened. Fixed by having every terminal branch do both. This is the second recorded fix to this same class of bug in the "done" handler (see the follow-up below) - flagged rather than treated as closed.

## 2026-08-11 - Rewind counts against disk, and the Stop button gets a client-side watchdog

Two independent fixes to the same "kill/rewind a running turn" surface:
- **Rewind** counted user messages against the in-memory pane, which can include a message sent seconds ago that the transcript file hasn't been written to yet - `forkTranscriptAtUserMessage` counts straight off disk, so rewinding to a not-yet-flushed message found no match and silently forked the whole file untouched. Fixed by withholding the rewind action from a still-pending user turn, and making the fork function fail loudly (not silently no-op) if ever asked to rewind to a message not on disk yet. Rejected: making the in-memory pane count match disk instead - the disk file is the actual source of truth for what a fork operates on, so the fix belongs on the read side, not on faking a count that would still race.
- **Stop** force-kills the process tree, but the pane only clears "Stopping…" once the CLI's stdio pipes close - some shapes of a Bash-tool descendant inherit those pipes without being tracked in the killed tree, so a successful kill can still leave the pane hung forever waiting for a pipe-close event that will never come. Fixed with a client-side watchdog that forces the pane back to idle if nothing terminal arrives within a few seconds of a successful kill signal, rather than trying to enumerate every process shape that can inherit a pipe.

## 2026-08-11 - Pending-turn buffer given a real lifetime instead of a fourth manual delete

Root cause of "the first reply in a long session re-appears at the bottom on every reload" (Jot 6bdbcde7): `mergeReloadedTurns` only matches a pending turn against the file's last 60 turns, so once a conversation passes that window an early pending turn becomes permanently unmatchable and gets re-appended on every reload. The only thing that had ever emptied the buffer was a hand-written delete added separately to each terminal branch of the "done" handler (see the entry above, and the one before it in this file) - the third time a leak was found and closed by adding a fourth call site. Per this project's stated preference for the deeper fix over another patch on the same shaky mechanism (CLAUDE.md, "proper fixes over patches-on-patches"): rather than find and patch the Nth call site, everything pending now expires when the NEXT RUN starts (`sendFromPane`), the single choke point every path must pass through to produce more output at all - a queued prompt routes through it too. Safe because by the time a new run starts, the previous process has exited and the CLI has flushed its transcript, so anything still unmatched at that point is not "the file is behind," it is a turn the file will never grow to contain. Rejected: finding and fixing the specific still-leaking branch (as done twice before) - a rule enforced at N call sites is one an N+1th call site can still silently opt out of; a single choke point has no Nth site left to miss.

## 2026-08-11 - The review page renders the whole review (not just the diff), gives commit-only rows a real body, and writes verdicts in the task's own language

Three related fixes to what the Review page actually shows, landed together because they're one surface:

- **"Present review" only rendered the diff.** The diff already has its own in-app viewer; what was actually cramped in a narrow panel was everything ABOVE the diff. Fixed by rendering the whole review record - warnings, summary, open question, caveats, evidence, declared gaps, acceptance criteria, manual test steps, every check with its real exit code/output, and the independent verdict - with the diff moved to the last section, sourced from `buildReviewsPayload` (not recomputed) so the page and the card can't disagree. A task with no commits is no longer refused - that's exactly the cosmetic-stamp case the page is most useful for.
- **A commit with no Jot task had no review record and got no body.** Rejected: fabricating a record for it. Fixed by leading with the same "no review record" language an unrecorded task row already uses, then showing everything git DOES know (author, date, size, full commit message - previously shown nowhere), plus a verdict if one exists and the same two actions a task row offers (Present review; an independent reviewer keyed by sha, with its own brief - "confirm or refute the author's claims" doesn't apply when there are no claims).
- **Verdicts came back in English regardless of the task's language.** Everything a reviewer reads is English and nothing told it otherwise. `src/lib/reviewLanguage.js` detects the language from the task's own title/description, and the brief now NAMES the target language explicitly ("Write the verdict in Swedish") rather than saying "the same language as the task" - the model resolves the vaguer instruction toward English every time. Register rules (lead with user-facing impact, explain internal names, no unexplained jargon) ship alongside an explicit carve-out: identifiers, paths, commands and output are neither translated nor dropped - plain language means fewer abstractions, not less evidence.

Deliberately left uncommitted alongside this: an unfinished "Bound to commits" review filter already sitting in the working tree - it belongs to separate, still-open work and was not folded in.

## 2026-08-11 - Resuming a direct/derived second mate must re-bind its dispatch identity, not rely on recovering it

Found by following a live session that ran every task itself instead of dispatching - its transcript had zero `helm_dispatch` calls and never got the delegate-vs-do manual, meaning it launched as a plain session, not a second mate. Root cause: `jumpIntoSecondMate` only set `secondMateId` on the fresh-launch path; resuming an existing session (`openSessionInPane`) relied on `secondMateForSession` to recover the id from the session itself, but a direct/derived second mate's session is never bound to begin with, so that lookup returns null and the resumed turn launches plain. Fixed by having `openSessionInPane` accept an explicit `secondMateId` override, threaded from `jumpIntoSecondMate`'s own value, and binding the session at that point so a later resolve/rebuild also recognises it. Rejected: only fixing `secondMateForSession`'s recovery logic - the actual invariant that broke is "the caller that already knows the identity must hand it down," not "the callee must always be able to re-derive it from scratch," and the latter has no reliable source for a derived second mate whose session was never explicitly bound.

## 2026-08-11 - The auto-captain gets its own dispatcher identity, distinct from the manual "direct" second mate

A manual captain second mate working a project, plus a separate Auto pass on the same project, made the manual one vanish from the Captain widget and reappear under Auto-captain. Root cause: both were keyed to `secondMateId("direct", project)` - one node per project, no distinction by who started it - so the auto run merged onto the manual node, and a sticky `startedBy: "auto"` flag flipped the whole node (including the manual session) into the Auto lane on the next repaint.

Fixed by giving the auto path its own dispatcher identity, `AUTO_CAPTAIN = "auto"`, distinct from the manual `"direct"`: the auto path dispatches under `secondMateId(AUTO_CAPTAIN, project)`; `deriveSecondMates` routes every auto-started run to the project's auto node - including a legacy run still dispatched under the old `"direct"` id, so an existing collision clears without rewriting history; and `"auto"` is treated as top-of-chain (reports to the captain, not up a first mate) everywhere `"direct"` already was. Result: a shared project gets two nodes, one per lane, matching two different sessions - not one node whose lane flips depending on which ran last. Rejected: keying by session id instead of by `(dispatcher, project)` - that would have avoided the collision too, but the existing per-project-node model is relied on elsewhere in the Captain widget, so changing the identity function rather than the node model was the smaller, more consistent fix.

A same-day ship-review of this fix (`ee56f2f4`) found one spot it missed: the review-nudge scoping in `renderer.js` checked `firstMateId !== "direct" ? ... : true` without the equivalent carve-out for `"auto"`, so every auto node's crew-review nudge rendered blank (the auto node's crew is dispatched under the auto second mate's own `sm_` id, never the literal `"auto"`). Fixed the same way - treat `"auto"` as top-of-chain at that site too.

## 2026-08-11 - Autopilot's persistent right-side notice replaces the fading toast for two events, and the crew-row iteration count is added to the dashboard's repaint fingerprint

Two small UX fixes landed as part of the same goal-orchestrator crew run:
- The "handoff saved" success message and the second mate's topic-guess caveat both used to fire as a fading bottom-center toast, easy to miss. Both now route through the existing `showNotice`/`noticeQueue` mechanism (already persistent + queued) with a new slide-in-from-the-right / fly-out-on-dismiss animation and a "good" tone for the success case, rather than adding a new notification mechanism.
- The Autopilot crew row's `iter N` label sat stale until switching views and back, because a new iteration on an already-running run didn't change the fingerprint string the widget dashboard's repaint gate keys on. Fixed by including the per-run iteration count in that fingerprint.

## 2026-08-12 - A quota/token-exhaustion stop is now a recoverable run state, not a deleted failure

Root cause of a lost-work incident (2026-08-11): when an autopilot iteration stopped because the captain's own Claude tokens/quota ran out, the SDK's usage-limit signal (a stdout banner, or an error envelope) never reached the quota classifier, so the iteration read as a generic "schema mismatch" failure. Two such failures in a row marked the run a REAL failure - non-resumable - and, having zero commits, DELETED its worktree, destroying the work with no resume path.

Fixed by having `runIteration` surface the SDK's own error result plus a stdout tail (rather than a generic message) into a classifier extracted as `isResumableQuotaError`, matching on the actual strings a usage-limit stop emits (usage limit reached / resets at / 429 / 529 / overloaded / insufficient / credit balance). A token/quota stop now classifies as `quota_exhausted` -> resumable, so the worktree is kept. Two follow-on pieces landed with it: the Goal card's one-click Resume button, previously offered only for "interrupted" (restart) runs, now also covers a quota-stopped run since it finishes as done+resumable; and a new `helm_resume_crew` lets a second mate resume its OWN crew's resumable runs without escalating to a first mate (the fleet-wide `helm_resume_fleet` stays first-mate-only), under the same per-run guardrails (resumable-once, worktree-on-disk, kill switch, budget, per-mate width cap).

A same-day independent ship-review of this fix found the new classifier over-matched: bare terms like "insufficient" and "limit reached" also match "insufficient permissions", "insufficient information", and "recursion/memory limit reached" - none of them a quota stop - which would keep a genuinely broken run's worktree and let it auto-resume in a loop bounded only by budget. Fixed by anchoring the loose terms ("insufficient" -> `insufficient (quota|credit|balance|funds|tokens)`; dropped bare "limit reached", already covered by "usage limit" + the `\d+-hour limit` pattern; `\bquota\b` word-bounded) and by taking the stdout tail rather than the head on the JSON-parse-failure path, so a usage-limit banner printed after partial output isn't missed. Rejected: leaving the classifier broad and relying on some other gate to catch a real failure - once `isResumableQuotaError` became the *sole* quota-vs-failure gate, precision there is the only thing standing between a real bug and an infinite resumable loop.

## 2026-08-12 - The budget/kill-switch path fails loud on write and fails closed on read when `metaHome` isn't absolute

Follow-up to a stray `DropboxDocumentsClaude/.helm-dispatch/budget.json` found on disk 2026-08-12: a `metaHome` whose path separators got stripped (by a shell/heredoc mangling a Windows path) would `path.join` into a RELATIVE budget path, silently creating a second, ignored budget/kill-switch file under the process's cwd. Had a mangled `metaHome` ever also reached the read path, `isKilled`/`isOverBudget` would consult the wrong (missing) file, read a fresh zero-spend/not-killed state, and silently disable the ceiling and the kill switch - the exact runaway condition this guardrail exists to prevent.

Fixed by asserting an absolute `metaHome` in `budgetPath`: the write path (`update`) now fails LOUD, before its own best-effort try/catch, so the signal can't be swallowed; the read path fails CLOSED (returns `killed: true`) so a mangled `metaHome` can never read back as safe. `.helm-dispatch/` was also added to `.gitignore` as a belt-and-suspenders against ever committing such a stray artifact. Rejected: silently normalizing or repairing a mangled path - a guardrail file's job is to be trustworthy, and a "best guess" repair would just move the silent-wrong-file failure mode one step further away rather than removing it; failing loud/closed makes the mangling visible instead.

## 2026-08-12 - The atomic writer retries its temp-file cleanup instead of a single silent unlink

The atomic writer removed its own temp file with one silent `fs.unlinkSync` call, which loses a race on Windows+Dropbox: the sync client can lock the temp file the instant it appears, the unlink fails, and the temp is left behind. Fleet state rewrites every ~5s, so each lost cleanup compounded - 1462 orphaned `.fleet-state.json.<uuid>.tmp` files had accumulated in the dispatch dir by the time this was found and cleaned (2026-08-12). Fixed with `bestEffortRemove`, which retries the unlink over the same backoff the rename already uses, treats `ENOENT` as success, and never throws - used on both cleanup paths (the on-before-rename abort, and a write failure). Rejected: a longer single retry-free wait before unlinking - a fixed wait either isn't long enough for a real lock or wastes time when there's no lock at all; retrying with backoff adapts to whichever is actually happening.

## 2026-08-12 - An Auto autopilot run now shows which model it actually used

An Auto/auto-captain run passes no `--model`, so its `model` field was always null and the UI could only ever show the generic "Auto model" label - no way to tell which model the CLI actually picked for that run. This was a squash-merge-and-complete of an in-flight autopilot branch (`helm/goal-3ed5b53f`) that had built the plumbing - `extractUsage` reading the resolved model id out of the SDK's `modelUsage` key, threaded through every iteration record, the run result, the persisted history record, and the renderer's rehydration + live-event mirror - but had stopped short of the display step (its own run hit `max_iterations` before reaching it). Completed by having `goalModelEffortLabel` render `"Auto -> <model>"` once a model resolves (from `run.latestModel` live, or `run.result.resolvedModel` once finished), falling back to "Auto model" until the first iteration reports one; explicit model picks are unchanged.

## 2026-08-12 - The "commits without a task" review floor becomes a set of acks, not a single pointer

Two unbound commits toggled forever in the "commits without a task" band instead of both clearing on "Reviewed": the review floor was one per-project watermark, and `listUnboundCommits` used `<watermark>..HEAD`, which silently assumes linear history. The two commits sat on DIVERGENT branches (each merged via its own merge commit, so neither is an ancestor of the other) - `cA..HEAD` still contains `cB` and vice versa, so acknowledging one always re-surfaced the other. Fixed by making the floor a SET: the initial baseline plus a per-project set of acknowledged commit shas (`commitReviewAcks`), with `listUnboundCommits` excluding everything reachable from ANY of them via `git log HEAD --not <baseline> <...acks>` (`--ignore-missing` so a floor sha a history rewrite made unresolvable doesn't error the listing) - divergent siblings then clear independently of each other. `acknowledgeCommit` now ADDS to that set (accepts one sha or many) instead of overwriting a single pointer, and "Reviewed all" acknowledges every shown commit, not just the newest. Rejected: keeping a single watermark and trying to special-case merge commits - the actual bug is that "reviewed" is a SET membership question ("have I seen this commit"), not a single cutoff point, once history can branch.

## 2026-08-12 - Jumping into a second mate never adopts a session another node already owns

Follow-up to the archived-session fix below: `jumpIntoSecondMate`'s fallback to `mostRecentSessionForCwd` (used when the second mate has no bound session of its own) could adopt a session belonging to a DIFFERENT node - a Captain "direct" second mate, or another first mate's second mate for the same repo - and binding it here would then clear it from its real owner via `bindSecondMateSession`, i.e. jumping into a freshly-created second mate could steal the Captain's active session for that project (the captain, 2026-08-12: "kommer den ta over en annan session ... under captain?"). Fixed by having the fallback only adopt a recent session confirmed NOT already claimed by another second mate (checked against `listSecondMates`); if that lookup itself fails, it starts fresh rather than risk a steal - a fresh draft is always a safe fallback, an accidental adoption is not. A genuinely unowned/loose session is still adopted, so the original reconnect behavior holds for that case. Auto runs live in worktrees with a different cwd, so they were never at risk here.

## 2026-08-12 - Archiving a session frees its second-mate binding instead of leaving it dangling

A freshly spun-up second mate inherited a stale binding to a session the captain had already archived for the same project - the second-mate id is deterministic per `(first mate, project)`, so a "new" second mate could still point at dead history, and jumping in resurrected the archived session instead of starting fresh. Fixed on three sides: `session:archive` now clears any second-mate binding pointing at the session being archived, reverting that node to a null-session "proposed" seat; `bindSecondMateSession(id, null)` reverts status to "proposed" (a session-less node was never really "created"); and `jumpIntoSecondMate` additionally ignores a bound session that reads archived/hidden even if the binding wasn't cleared (belt-and-suspenders for a session archived from a different Helm instance the local binding hasn't heard about yet). Rejected: leaving the dangling binding and only fixing `jumpIntoSecondMate`'s read side - the write side (archive) is where the state actually goes stale, so leaving it uncleared would keep producing the same dead binding for every future jump-in check to work around.

Same pass added a first-mate-manual rule for a second, unrelated flow gap: asked to "spin up second mates for A and B and review them," a first mate had been creating the seats (`helm_create_second_mate`, which only REGISTERS - no work runs until the captain jumps in) without ever relaying the actual work (`helm_relay_to_second_mate`'s job), leaving empty seats the captain had to dispatch by hand. The manual now states the create-vs-relay split explicitly: a verb-first instruction ("review/fix these") relays each project (spins up AND works AND reports); create is reserved for "I want to work on it myself."

## 2026-08-12 - A record-less review card gets a real body: autopilot writes one, and the diff/reviewer no longer require one to exist

A task moved to review with no review record was a dead end - the card said only "nothing was written down," with no description, no test steps, and critically no way to send an independent reviewer, which was strictly LESS than an unbound commit's row got. Two separate causes:

1. `finishAutoRun` moved the task to review and wrote a Jot NOTE (what it did, where the worktree is) but never a review RECORD - and the card renders the record, not the note. Fixed with a new `buildAutoReviewRecord` that writes one from the run's own result, for any run that produced commits (a zero-commit run has nothing to review). Deliberately a `judgment` record, never a `stamp`: autonomous output is the machine's own claim, not a verified result, so the record carries an explicit, prominent `notVerified` list plus a check (the run's own verify gate, or an honest "commits present" check) - the card now shows what happened and offers a path to a decision, instead of a dead end.
2. The three useful actions on a review card - see the diff, present the whole review, dispatch an independent reviewer - were all gated behind `if (row.record)`, so a record-less card offered NONE of them, including the one action (send a reviewer) that would actually produce the missing record. Fixed by gating on `row.repoPath` (resolved from the category binding OR the auto-run's own project) instead - the diff and the reviewer only ever needed the project, not a record. `reviews:diff`/`reviews:reviewerPlan` gained a project-path fallback parameter so an unrecorded task's row can still resolve one.

Rejected for (1): fabricating a `stamp` record so the card looked "complete" - that would misrepresent unverified autonomous output as already-checked, exactly the failure mode `docs/review-pipe-status.md`'s criticality gradient exists to prevent.

## 2026-08-12 - The fleet spend ceiling becomes an explicit, off-by-default Settings toggle

The $ ceiling on Helm's own estimated fleet spend (`orchestrationBudget.js`) has been live since Phase 2, including a `setCeiling(null)` escape hatch to remove the cap entirely - but nothing in the UI ever called it, so the only way to change or disable it was to hand-edit `budget.json`. The captain, 2026-08-12: on a Claude subscription there is nothing to cap by default, so the guardrail should default to off, with a place to turn it on and set the amount if he ever wants one. Added a "Fleet guardrail" Settings group (a toggle plus a $ input) that round-trips through the EXISTING `orchestration:setCeiling`/`orchestration:budget` IPC - no new backend needed, since the capability was already there and simply unreachable. The manual Stop button on the Dashboard (the separate `killed` flag) is unaffected either way - it was never gated on a ceiling existing. Rejected: adding a config.json field for "ceiling enabled" separately from `ceilingUsd` - the existing `null`-means-off convention in `orchestrationBudget.js` already encodes exactly that state, so a second flag would just be two sources of truth for one fact.

## 2026-08-12 - A stashed WIP is verified and finished rather than re-derived from scratch

Found via `git stash list` during a routine review pass (a session had ended abruptly the prior night, "det blev lite strul med autopilots" - see the crash-recovery entries above): one stash containing two already-complete features (the review page's "Bound to commits" filter, and the quota widget's Resume/Stop button). Rather than either discarding it unseen or re-implementing the same features fresh, the stash was applied, the fast suite run against it (79/79 green, unchanged), and it was committed as-is - confirming a session that ends abruptly on a legitimate, working change should have that change recovered, not lost to "safer to redo it." General lesson for reviewing another session's leftover state: verify-then-keep beats discard-and-redo whenever the diff is self-contained and the test suite can actually confirm it still holds.

## 2026-08-12 - Reconciling a stash/commit with a LIVE concurrent editor of the same working tree: stage only the owned hunks, never `git checkout` the shared file

While applying the stash above, a SEPARATE live session turned out to be actively writing to the exact same files (`renderer.js`, `preload.cjs`, `main.js`) in real time - confirmed by watching `git status` gain new, unrelated hunks between commands with no action taken in between (its own commit, `c3ad97f`, landed cleanly afterward). This is a known-live risk for this project (see "Auto-lane sessions see an unstable git view," memory), but hadn't previously been walked through end-to-end for a two-writer collision in the SAME literal directory (not just an unstable read). The pattern used, now worth keeping as the reference: never run a bulk `git checkout -- <file>` on a file another session might be mid-write on (it would discard whatever they'd written since your last read with no recovery); instead extract exactly the hunks you own into a patch (diffing a HEAD-blob copy against a hand-edited copy of it gives clean context, avoiding line-number drift against the live working tree) and `git apply --cached` only that patch, leaving every foreign hunk sitting untouched in the working tree for its own owner to commit. Confirmed safe here: the other session's commit landed intact afterward, and this session's three commits landed intact too - neither lost anything. Already the established pattern in this repo's history (`9510ba64`, `ee56f2f4`) for a single foreign hunk; this is the same discipline holding under an ACTIVE, moving target rather than a static leftover diff.

## 2026-08-12 - The dispatch queue's acks/reports get a pruner; a sibling inconsistency in `writeAck` gets fixed alongside it
...
**Why, and this is the part that matters.** The captain, 2026-08-16, stating an original goal of
...
touch code. That is the wrong trade for the captain, who wants to be hands-on. Same shape, opposite
route.
...
- *Kun Chen's one-interface model (captain speaks only to the coordinator).* Rejected: it
  removes the hands-on work the captain values and is good at.
...
4. Fewer seats do not remove the wait that drives the behaviour - they can convert stress into
   ...
   nothing needs you". Reviews is the surface the captain says he actually spends that wait on, so
...
Settled with the captain the same day, refining the direction entry above.
...
relaxation is a per-project `yolo` posture the captain approves (AGENTS.md:29, 280-284). The captain
proposed the per-project toggle independently.

**Merging lands; it never publishes.** The captain's instinct was auto-merge; his answer to "what
...
Worth noting what firstmate does with worktrees, since the captain asked: it DOES tear down
...
said "100% for two weeks", and the captain rejected the shape, not the number: "då måste jag komma
...
project switches per day, and one line from the captain on whether the week felt calmer or merely
...
Failing a gate is not a verdict on the work, it just routes it to the captain. The useful property
...
**A new project is born as a seat, not by a session moving.** The captain asked whether a seat in the
...
the limit. So a seat PROPOSES a project seat and it appears as a proposal for the captain to place.
The scarcity stays his.

**Budget: cap concurrent OPUS, not tokens.** The captain's framing, and it is the right one: "Sonnet
...
**Recommendation: adopt, do not rebuild** (the captain's standing rule about not re-implementing
...
The captain asked why crew agents "always get Haiku" and solve bugs half-heartedly.
The premise was wrong and what was actually happening was worse.

**They were never on Haiku.**
All 22 skiff crew runs requested and ran Opus 4.8; the cost proves it ($19.72 for a run
...
The captain's hypothesis - that Sonnet would have sufficed - is untestable on the existing data,
because every run was Opus with amputated notes.
It becomes testable from here.

## 2026-08-18 - the captain does not trust Helm, and the reason is structural rather than a run of bad luck
...
**Consequence worth stating plainly:** the skiff board is probably more optimistic than the
...
4. **Nothing dies silently** (already on the board as the watchdog item) - because most of
   these were found by the captain noticing, not by Helm noticing.
...
The captain: "jag vill inte köra helm förän vi lagt ner tid på att fixa den. Jag kör mina sessioner i
...
main. It also cannot reach the captain's own Helm, whose profile is `%APPDATA%\helm`.
...
Quota exhaustion is 0 of 51 while the captain's own notes record autopilots dying of it.
...
Settled with the captain after the measurement showed the term had drifted.
...
This must NOT simply be deleted along with the flag. Nothing else currently surfaces unreviewed work, and 117 crew commits reaching skiff's master unreviewed is the concrete cost of not surfacing it.
