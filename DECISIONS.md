...
**What this entry does not settle**, and is recorded as open rather than guessed:
which id namespace the merged seat lives in; where `helm_report_up` points once there is no tier above a project seat; what happens to sessions currently held by the direct widget when it is removed; the model-per-tier assignment, which had the cross-project seat on a cheaper model and now has only the project tier left; whether flipping the `keepPersona` default is sufficient or assistant-ness needs storing where a respawn structurally cannot drop it; and the merged seat's write budget. That last one is easy to miss because it looks like a constant: `SECOND_MATE_TURN_WRITE_BUDGET` caps the seat at three writes a turn, and it exists to stop an orchestrator quietly doing its crew's work. Keeping the name and dropping the budget is a far larger change than a rename, and keeping it on what is now the top working tier may be too tight. Each of these is a real fork, and none was part of what was decided.
